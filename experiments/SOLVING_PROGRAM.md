...
## Session update 2026-06-01g (the order-2 symmetry-reduction program, built to its exact frontier; L46-L50)

The measurable block-diagonalized SDP (flagged in update-f) was built in full, both reductions, and pushed until it hit a precisely-characterized wall.

- **Order-1 (e3p, L46).** $S_k$-block-diagonalized: the symmetrized order-1 PSD ($1+nk$) reduces to a vacuous trivial block plus an $n\times n$ standard block, $M\succeq0 \iff S\succeq0$, INDEPENDENT of $k$. Reproduces e3m exactly on all small configs (no fake certificate); X_23 $k=4$ in ~3 s (23x23) vs e3m's ~290 s (93x93).
- **Order-2 (e3q, L47).** $S_k$-block-diagonalized via the Murota multiplicity alignment (the naive eigenbasis provably fails: the order-2 rep has irreps with multiplicity $>1$; alignment residual ~1e-13). Reproduces e3n on triangle+rhombus (no fake cert), runs Moser order-2 in 6 s where the naive e3n OOMs at 25 GB; PSD blocks independent of $k$ (rhombus $146\to$ max 17), 50-380x faster.
- **O(2)-congruence reduction (L49).** Identifies congruent colored sub-configs as one moment variable ($\equiv$ size-$\le4$ IEC, structurally). Validated against e3n+IEC.
- **The honest boundary (L48/L49/L50).** X_23 order-2 has ~98627 moment variables. $S_k$ shrinks the PSD blocks but not the variable count; $O(2)$ halves the variables only ($98627\to48342$, because X_23 is geometrically generic at the 4-point level). So the two reductions TOGETHER do not fit X_23 order-2 in a dense-assembly solver (171 GiB affine map), and an intermediate-config probe found order-2 never strictly beats order-1 at runnable scale (no shortcut to validate the lift).

**Net.** The full measurable stack (e3k$\to$e3l$\to$e3m$\to$e3n$\to$e3p$\to$e3q+Parts 1&2) is complete and validated, and its frontier is mapped: the measurable $\chi_m\ge6$ attack reduces to ONE well-defined prerequisite, a custom SPARSE conic backend (de Laat-Vallentin / DeCorte-Oliveira-Vallentin). Symmetry alone is proven insufficient. No bound moved.
...
| 7-mc | Multi-class measurable LP for chi_m >= 6 (the one un-capped route) | prototyped (e3k, L38); IEC layer (Formulation 1+2) BUILT+VALIDATED (e3l, L39); SCALABLE moment/Lasserre backend BUILT+VALIDATED (e3m, L40): degree-1 color-marginal moments + order-1 PSD matrix + Bochner + IEC keys (linear on moments), never enumerates colorings. Cross-validates vs e3l, passes chi_m<=7 gate, scales to 19 pts in seconds. Both L38 barriers now cleared at the infrastructure level. Lesson: solver accuracy is load-bearing (SCS faked a chi_m>=6 cert at 1e-5; CLARABEL kills it). X_23 RESTORED+TRACKED + run end-to-end (L43): degree-1 IEC too weak (k=4 margin 0). FULL symmetry-reduction program then BUILT+VALIDATED: lossless S_k symmetrization (L44), S_k-block-diag order-1 (e3p, L46) + order-2 (e3q via Murota alignment, L47, runs Moser where naive e3n OOMs), O(2)-congruence variable reduction (L49). HONEST BOUNDARY (L48-L50): both reductions together insufficient for X_23 order-2 in dense cvxpy (48342 vars x blocks up to 735); order-2 never strictly beats order-1 at runnable scale. | sole blocker now: custom SPARSE conic backend (de Laat-Vallentin); symmetry alone proven insufficient |
