...
| E13/E13b small $\omega=3$ host factory + codegree wall | 1 | done; FACTORY: Mycielski-tower seeds ($M(M(W_5))$ 27 vtx, $M(M(C_5)+K_1)$ 25 vtx) + SAT-guided shrink give K4-free 6-critical triangle-rich hosts down to **n=18** (84 triangles covering all vertices); alternators ABUNDANT (438-706 per core, ~20-26% of disjoint pairs vs 2.8% on $M^3(C_5)$); pair-swap involutions present (n=20 core, $\|\mathrm{Aut}\|=10$). WALL: every UDG is $K_{2,3}$-free (two unit circles share $\le2$ points) $\Rightarrow m \le n(1+\sqrt{8n-7})/4$; ALL ~35 edge-critical cores land at $m/n \approx 4.2$ with 126-148 codegree violations (n=18 edge-critical m=76 > ceiling 57); $M^3(C_5)$ has 434 violations (max codegree 11). Rigorous: any 6-chromatic UDG has a 6-critical core with $n \ge 13$. Conditional: density ~4.2 pushes UDG-plausible hosts to $n \ge \sim32$. Next: E14 generate INSIDE the both-free class from $P_{510}$. Scripts `combinatorial/e13_small_hosts.py`, `e13b_edge_minimal.py`; hosts in `e13_hosts.json`/`e13b_hosts.json` (LEARNINGS L63) |
| E14 generate inside the UDG-necessary class (from P510) | 1 | IN PROGRESS, verdict OPEN; seed logic CONFIRMED ($P_{510}$ is natively $K_4$-free AND $K_{2,3}$-free); class is LIQUID (674 codegree-safe edges added to $P_{510}$, still 5-colorable, no stuck, far below $\sim$8200 codegree ceiling). KEY METHODOLOGICAL FINDING: portfolio inversion -- Cadical 12+ h vs MapleChrono 155 s on the SAME 470-edge instance ($\ge$280x), retroactively explaining the h6 instance abandoned in L29/L30; near-threshold instances MUST be a solver portfolio. Overshoot method (jump past the wide hard band with blind safe edges, solve cheap on the overconstrained side) validated in shape; UNSAT-vs-STUCK verdict pending. Scripts `combinatorial/e14_udg_class_host.py`, `e14b_overshoot.py`, `e14c_overshoot_jump.py`; write-up `docs/03_research/E14_UDG_class_and_portfolio_findings.md` (LEARNINGS L64) |
