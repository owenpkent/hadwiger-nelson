...
### L66. Nogood/clause learning added to hn_solver (CBJ + nogood store): it WORKS algorithmically (6.7x fewer nodes) but is wall-time-NEUTRAL in pure Python, empirically proving learning and a compiled-language port are COMPLEMENTARY levers, not substitutes. New module [`hn_solver_cdcl.py`](_shared/hn_solver_cdcl.py): conflict-directed backjumping (Prosser CBJ) + nogood learning (each conflict set recorded as a forbidden (vertex,color) combination, indexed and reused to prune). Validated CORRECT: zero verdict disagreements vs the chronological hn_solver AND the MapleChrono portfolio over 200 random graphs x 3 colors, plus valid witnesses. Key soundness fact that makes conflict analysis clean under symmetry breaking: a vertex can always take color $(\max\text{ used})+1$ unless that hits $k$, so a domain wipeout occurs ONLY when the vertex's neighbors already occupy all $k$ colors -- a pure graph conflict needing no symmetry explanation (so the conflict set is exactly one neighbor per color).

**The measured result (the point).** On $M^3(C_5)$ $k=5$ UNSAT: chronological hn_solver expands 447,720 nodes; CBJ-alone (no nogoods) expands 516,549 (WORSE, because keeping CBJ sound forced a fixed degree-order instead of MRV, and that ordering loss outweighs backjumping); CBJ + nogood learning expands **66,651 nodes, a 6.7x reduction**, and slows the per-Mycielski-level explosion from ~500x (chronological $M^2\to M^3$) to ~68x. BUT wall time is unchanged (14.3 s vs chronological 14.4 s, 1.01x): the nogood checking makes each node ~6.7x more expensive in Python, exactly cancelling the node savings; $M^4(C_5)$ $k=6$ stays intractable (<2,000 nodes/s). So learning lowers the node count (algorithm) while a compiled language would lower the per-node cost (constant factor); neither alone cracks $M^4$, both together would. This validates the L65 white-paper analysis and the Rust discussion with a hard measurement.

**Architecture**: 1 (combinatorial / solver tooling). Module + self-test [`hn_solver_cdcl.py`](_shared/hn_solver_cdcl.py); white-paper room-for-improvement #1 updated with the measured outcome. **Honest scope.** No bound moved; a tooling result. The remaining in-Python lever is watched-literal nogood propagation (cut the per-node nogood-check cost that eats the win); beyond that, a Rust/PyO3 core. The learning is real and correct; the Python constant factor is now the demonstrated bottleneck.
