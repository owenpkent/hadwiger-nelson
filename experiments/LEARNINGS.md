...
**Scope.** $\chi=4$, not 5; no bound moved; whether this core extends to a $\chi=5$ object in the same field is exactly the open follow-up. The radius-3 sweep otherwise reports 8 fields, 0 reaching $\chi\ge5$, with the six inert fields all landing on the identical 865-point / 3,942-edge ball.

**THE $\chi\ge5$ RUNG IS NOT CALIBRATED, so that last negative is UNINFORMATIVE and must not be cited.** The calibration for it is the Moser field itself: de Grey's 1581-vertex $\chi=5$ graph lives in $\mathbb{Q}(\sqrt3,\sqrt{11})$ and is connected with unit edges, so SOME ball of that field must be non-4-colorable. Measured, every ball this module can build there is 4-COLORABLE: radius 4 over 18 vectors (3,313 points, 21,900 edges), and with the rotor's POWERS included (the tower the real constructions use, not one rotated copy) $K{=}2$ radius 3 (4,141 points), $K{=}3$ radius 2 (883 points), and $K{=}3$ radius 3 (**11,593 points, 66,978 edges**) -- all 4-colorable. So the generated vector system does not contain de Grey's graph, and the unit vectors his construction uses are not exhausted by $\langle \zeta_6, \text{rotor}\rangle$. **Consequence: E24 is calibrated at $\chi\le4$ only. Its positive findings (both binding fields, the 56-vertex core) stand, being verified directly; its $\chi\ge5$ negatives establish nothing, because the method has not been shown able to find a $\chi=5$ object even where one provably exists.** Fixing this means identifying the actual generator set of the lineage's vector system, which is now the gating sub-problem for the whole ring-census idea.
...
**E24, the ring census.** Exact biquadratic arithmetic (4-tuples of Fractions over $(1,\sqrt a,\sqrt b,\sqrt{ab})$, so "distance exactly 1" is an identity, never a float compare). Calibration passes with the WRONG-APPROACH DETECTOR AS RUNG ONE: $\mathbb{Q}^2$ with Pythagorean unit vectors is $\chi=2$ at radius 2 and 3 (Woodall), triangular lattice $\chi=3$, Moser field $\chi=4$ at radius 2. That last rung failed first and caught a real bug: rotating only $(1,0)$ instead of the whole generating set leaves the rotated rhombus unreachable, so the spindle cannot appear. Radius-2 sweep: 8 fields, **0 reaching $\chi\ge5$**, and the equal-ball observation above. Reading: the Moser field is distinguished not by size but because its rotated copies CLOSE UP (the binding of L11/L14, now quantified at 36 edges). Radius-3 sweep running.

**E20 $n=18$ (in progress, honest interim).** 4 of 9 cells closed (m=54..57, the densest); the 5 sparse cells m=49..53 each exceeded a 4 h budget and are now in recursive splitting, with m=49 already at depth 10. My earlier 40x-per-order projection was too optimistic: $n=18$ is costing far more than $n=17$'s 0.83 cpu-h, and the branch timeout has been cut 14400s -> 1800s so the split tree deepens instead of burning hours per doomed branch. $n\le17$ is unaffected and remains the published claim.

---

### L80. E22: the E20 verdicts CANNOT be certified by a DRAT/LRAT checker, and the obstruction is structural rather than a tooling gap: $\Phi_{n,m}$ IS SATISFIABLE (its models are the class members), so no propositional refutation of it exists to be checked. The very fact that makes an UNSAT meaningful (the non-vacuity control) is what makes it uncertifiable in that format. Corrects a claim L78's paper amendment had made an hour earlier.

**The evidence, three ways.** `smsg --lrat-output` does emit a proof (5.2 MB for the $n=15$ $m=41$ cell; binary by default, `--cadical-config no-binary` for text). An independent checker (`lrat-check`, built from drat-trim) REJECTS it. And plain CaDiCaL on the same $\Phi_{15,41}$ without propagators returns a 41-edge model. The rejection is correct behavior: what smsg reports is unsatisfiability of $\Phi_{n,m}$ PLUS a semantic side condition ("no proper 5-coloring") that is not part of the formula.

**What a certificate has to be.** Three families discharged separately: (1) each COLORING clause is justified by exhibiting the 5-coloring that produced it, checkable in $O(n^2)$ by confirming the pairs it lists are exactly that coloring's monochromatic non-adjacent pairs; (2) MINIMALITY clauses are justified once and for all by the Kirchweger-Szeider lex-minimality theorem, a proof about the propagator, not a per-run artifact; (3) the CDCL remainder is genuinely LRAT-checkable given (1) and (2) as axioms. The cleaner route is to stop hiding the universal quantifier: the question is natively $\exists G\,\forall c$, a 2-QBF, and `smsg` accepts QCIR for exactly that fragment. Putting it there moves the universal half inside the formula where a certificate can reach it.

**Consequence.** C1's epistemic-status paragraph now says precisely what is and is not certifiable, instead of promising an LRAT hardening that cannot work as described. No bound moved; $n\le17$ unchanged.

---

### L79. E21: the both-free class is strictly weaker than unit-distance geometry allows, and the missing constraint prunes 64% of it -- yet adding it makes the E20 decision procedure 2x SLOWER. A constraint worth a $3\times$ speedup to an ENUMERATOR is worth nothing to a DECIDER, which is L78's decoupling claim confirmed from the other side.

**The geometry (new content).** $N(v)$ lies on the unit circle about $v$, and two points there are at distance 1 exactly when they subtend $60^\circ$. So edges inside $G[N(v)]$ join angles differing by exactly $60^\circ$, and a cycle inside a neighborhood is a closed walk of $\pm60^\circ$ steps. Hence (N1) every vertex of $G[N(v)]$ has degree $\le2$; (N2) NO ODD CYCLE lies in a neighborhood and $C_4$ forces a repeated angle, so the only surviving cycle is $C_6$; (N3) one component uses one residue class mod $60^\circ$ and there are six, so EVERY COMPONENT OF $G[N(v)]$ HAS AT MOST 6 VERTICES. Codegree $\le2$ already implies (N1), no-$C_3$ ($=K_4$-free) and no-$C_4$ (opposite vertices would have three common neighbours counting $v$); it does NOT imply no-$C_5$ (exactly $W_5$-freeness), no-$C_7$, or (N3). L25 already knew $W_5$ is not a UDG; it was never made a constraint on the search.

**Both gates pass before anything was built on it.** NEW: $W_5$ is $K_4$-free with max codegree 2, so the current class admits it, yet it is not a UDG. TRUE ON REAL UDGs: de Grey 1585, Heule 826, Heule 874 all show max neighborhood degree exactly **2**, largest neighborhood component exactly **6**, zero odd cycles, zero non-$C_6$ cycles -- the claims survive and BOTH BOUNDS ARE TIGHT.

**The measured negative.** Brute-force "forbid every (hub, $C_5$)" costs 576,576 clauses at $n=16$ and runs 5x slower. Recast as BIPARTITE NEIGHBORHOODS (witness the 2-colouring with $n(n-1)$ side variables instead of enumerating cycles) it is strictly stronger (kills all odd cycles) at 3,360 clauses -- and still **2,523 s vs 1,200 s on the hard $n=17$ $m=47$ cell**. Not because it is vacuous: checked against the committed $n=15$ artifact, **7 of the 11 class members are excluded**. It removes most of the class and the search still slows, because E20's cost is decoupled from class size; cutting the class only adds clauses and free auxiliary variables to branch on.

**Where it should go instead.** To the GENERATOR, not the decider: E17's `geng` prune pays per graph produced, so a filter excluding 64% is worth roughly $3\times$ there ((N2) into `e17_prune.c`). The mathematics belongs in C1 regardless of timing, as a strictly stronger UDG-necessary condition than the pair the note currently uses. Artifacts: `combinatorial/e21_neighborhood.py`, `combinatorial/e21_e22_results.md`.

---

### L78. E20: the $\Sigma_2$ question does not have to be answered by enumeration. Folding the colorability half INTO the solver (SMS's chromatic-number propagator, which the program had never used) collapses "does the both-free class contain a $\chi\ge6$ member on $n$ vertices?" into ONE CDCL run whose cost tracks search difficulty rather than class size. **$n=17$ is CLOSED: all seven cells UNSAT, 0.83 cpu-h**, against the $>80$ cpu-days L75 measured for the enumerate-then-color route. The $n=16$ verdict reproduces in 74.3 s against 66.3 cpu-h ($\sim$3200x). Any both-free host, hence any host for a $\chi\ge6$ UDG clamp, now has $n \ge 18$.

**The mechanism.** `MinChromaticNumberChecker` (`src/graphPropagators/coloringCheck.cpp`) is a lazy constraint: on a fully-defined candidate it looks for a $(k-1)$-coloring, and on finding one it learns "at least one of the currently monochromatic non-adjacent pairs must become an edge" and hands the conflict back to CDCL. So the $\forall$-half of the $\Sigma_2$ question becomes clause learning inside the same solver whose minimality propagator already owns the $\exists$-half's isomorphism problem. `smsg -v n --min-chromatic-number 6 --dimacs <cell>`: `Result: 20` = no member of the cell has $\chi\ge6$ (the theorem), `Result: 10` = a hit. The class is never enumerated; not one graph is emitted.

**Why this was available and unused.** L77's own build note records patching "two POSIX `random()` calls in an unused coloring propagator" -- the tool shipped with the lever the program was looking for, in a file the program had read and dismissed. E19 was written to enumerate 1.15M+ classes in the $n=17$ $m=46$ cell via a 64-way resumable branch driver; E20 decides that same cell in 736 s.

**Soundness is unchanged from L75/E17, no new burden.** Both class properties are subgraph-closed, so a $\chi\ge6$ member implies a 6-critical member on $n'\le n$ vertices, which has $\delta\ge5$, is connected, meets Kostochka-Yancey, and meets the E17 maxdeg lemma. Same relaxation-gated `CriticalEncoding` cells as E18/E19, no lex-leader clauses. SMS minimality clauses only exclude non-lex-minimal labellings (and a weaker cutoff prunes LESS, never more), and a learned coloring clause excludes exactly graphs on which the witnessing coloring stays proper, all of which are 5-colorable. Every SAT model is re-verified by disjoint code (E17 filter + the repo's SAT portfolio).

**Calibration, all rungs pass, nothing trusted before it.** (A) EXTERNAL UNIQUENESS THEOREM, both directions: triangle-free $\chi\ge4$ is UNSAT at $n=10$ and SAT at $n=11$, and the model returned is **isomorphic to the Groetzsch graph** (Chvatal 1974). (B) $K_4$-free $\chi\ge5$ UNSAT at $n=10$, SAT at $n=11$ (Jensen-Royle 1995; reports rather than gates, being literature-dependent). (E) THE PROPAGATOR AT THE PRODUCTION VALUE $k=6$, which A and B do not reach: since a $k$-critical graph has $n=k$ or $n\ge k+2$ but never $k+1$, "$K_6$-free, $\chi\ge6$" must be UNSAT at $n=7$ and SAT at $n=8$; both land in 0.01 s and the $n=8$ model is **isomorphic to $C_5 + K_3$**, independently confirmed $\chi\ge6$. (C) NON-VACUITY per production cell: the same CNF at $\chi\ge3$ must be SAT with a model the disjoint E17 filter certifies as a class member (an UNSAT on an empty cell is information-free; the one empty cell found, $n=15$ $m=43$, is correctly flagged rather than silently counted). (D) THE REPO'S OWN ANSWER by a third route: the $n=15$ and $n=16$ windows return UNSAT, reproducing L75 (geng) and L77 (SMS census).

**Measured.** $n=15$: 3.6 s vs 420 s for the geng gate re-measured on this host today. $n=16$: 74.3 s vs 66.3 cpu-h. $n=17$: 1202 s wall / 2975 cpu-s on 7 jobs, per-cell $m=46..52$ = 735.7 / 1200.4 / 777.8 / 170.9 / 53.4 / 23.7 / 13.3 s, zero hits, zero unknowns. Cost per cell grows roughly 2-3x per unit DECREASE in $m$, so the sparse end of each window is where the ladder will eventually wall.

**Scope, stated plainly.** This bounds the CLASS, not $\chi(\mathbb{R}^2)$. W3 (unit-distance realizability of the clamp) is untouched and the L63 codegree wall stands. What moves is exhaustiveness: C1's "no small host exists" claim gains an order, and the case that the missing object needs a NEW construction principle (rather than more search at small $n$) gets stronger. It also does not close L75 caveat (ii): the counting lemmas remain formalization-ready but not Lean-proved.

**The ablation, and a structural bonus.** At $n=17$, $m=52$: full class UNSAT 13.3 s; drop $K_4$-freeness, still UNSAT 17.9 s; drop the codegree bound ($K_{2,3}$ allowed), UNDECIDED at the 900 s cap; drop both, **SAT in 409.2 s with the model independently verified $\chi\ge6$**. The last row is the end-to-end POSITIVE CONTROL and it PASSES: in the same $n$/degree/edge regime the pipeline does return SAT, so the unablated UNSAT is a fact about the class rather than a failure of the search. The two middle rows localize the obstruction: **at this order it is $K_{2,3}$-freeness, not $K_4$-freeness, that forces 5-colorability** -- dropping $K_4$-freeness costs 5 seconds, dropping the codegree bound leaves the cell undecided. Direct support for L69, and a pleasing coincidence for C1's thesis: the constraint that comes from planar unit-distance geometry is also the one doing the combinatorial work.

**Infrastructure.** [`combinatorial/e20_sigma2.py`](combinatorial/e20_sigma2.py) with `--calibrate`, `--sweep`, `--probe` (non-vacuity), `--ablate` (the ablation above), and `--climb` (the always-on ladder over $n=18,19,20,\dots$, resumable at cell granularity, auto-splitting a timed-out cell into 32 cached branches, stopping loudly on the first hit or the first genuine wall). Results: [`combinatorial/e20_results.md`](combinatorial/e20_results.md), `_cache/e20/`. `smsg` now also builds on the Linux host (SMS `63958bd`, bundled `cadical_sms`, Boost 1.83 installed user-local; no root).

---

### L77. E18 closure: SAT MODULO SYMMETRIES kills the blocking-tail wall on contact -- the $m=43$ cell that two split campaigns could not close in hours of grinding is EXHAUSTED in 45.4 seconds, with exactly one emitted model per isomorphism class. The independent $n=16$ census is now COMPLETE: 11,315/11,315, matched against geng GRAPH FOR GRAPH, verdict AGREE, zero property violations, all six cells proved exhausted. L75's caveat (i) is now FULLY closed at $n=16$: enumeration completeness rests on two agreeing enumerators of genuinely different technology (canonical augmentation vs CDCL with in-solver canonicity), not on geng alone.

**The tool and the build.** Kirchweger-Szeider SMS (`smsg`, github.com/markirch/sat-modulo-symmetries) does lex-minimality checking INSIDE the CDCL solver via a lazily-refined propagator, so non-canonical labellings are pruned rather than enumerated-and-blocked. Built tonight on the Windows/msys host under ucrt64 (bundled `cadical_sms` as a static lib; two portability fixes: skip the `mobical` target, needs `sys/mman.h`, and patch two POSIX `random()` calls to `rand()` in a coloring propagator this use never touches; plus one msys2 partial-upgrade DLL trap, fixed by `pacman -Su`). Binary at `~/.local/src/sat-modulo-symmetries/build/src/smsg.exe`; needs `C:\Tools\msys\ucrt64\bin` on PATH.

**The driver and the soundness structure ([`e18_sms_cell.py`](combinatorial/e18_sms_cell.py)).** The CNF fed to smsg is the SAME relaxation-gated `CriticalEncoding` the blocking enumerator used ($K_4$-free, codegree $\le2$, $\delta\ge5$, $\Delta\le7$, $m$ pinned), with NO lex-leader clauses: SMS owns the symmetries. Variable-order compatibility is asserted, not assumed (smsg expects edge var $(i,j)$ at the lex rank of the pair, which is exactly the order `CriticalEncoding` allocates). SMS minimality clauses only ever EXCLUDE non-lex-minimal labellings, so no isomorphism class can be lost; duplicates, possible under the default check cutoff, are harmless because the downstream WL-plus-exact isomorph rejection and the disjoint-code property re-check still run on every emitted graph. "Exhausted" means smsg terminated with final UNSAT after the last solution, not a budget or timeout stop.

**Calibration before trust, 8/8 exact.** The three $n=15$ cells return 9/2/0 classes in ~1 s each, and the 11 graphs match the committed [`e18_n15_class.json`](combinatorial/e18_n15_class.json) 11/11 graph-for-graph; the five known $n=16$ cells return 645/75/10/1/1 ($m=44..48$), all EXHAUSTED, in 1.8-16.2 s. Models-per-class is **1.0 throughout** -- the quantity the blocking enumerator measured at ~250-370 unsplit and ~14 under its best split is simply gone.

**The main result.** $m=43$: 10,583 solutions $\to$ 10,583 classes, EXHAUSTED, 45.4 s, 0 property violations, 0 non-biconnected. `e18_merge` then reports CENSUS COMPLETE (every cell covered), and `e18_n16_compare` gives **matched 11,315, only-in-geng 0, only-in-SAT 0, property violations 0: AGREE (independent enumerations identical)**. For scale: the same cell defeated an 8-way split (5 of 8 branches ran past 540k-760k blocked models without closing) and a 64-way split (heavy tail past 100k), which is what L76 measured as the ~$10^5$-blocking-clauses-per-solver wall.

**What this validates for the $n=17$ door.** The SAT side's own wall (the blocking tail) is eliminated, so SMS is the confirmed right-hand enumerator for any $n=17$ attempt, as L76 prescribed. What SMS does NOT change: the class is ~100x larger at $n=17$ (L75's measured geng wall), and the $\Sigma_2$ colorability half is untouched, so $n=17$ remains a compute decision, not a tooling gap on the enumeration half.

**Architecture**: 1 (combinatorial). **Wrong-approach detectors**: inherited from E17/E18 unchanged (the class premise is Euclidean strict convexity; nothing here lifts to $\mathbb{Q}^2$ or $L^\infty$). **No bound moved**; this bounds the CLASS and hardens L75's verdict; W3/realizability untouched; L75 caveat (ii) (Lean formalization of the two counting lemmas) untouched. **Housekeeping**: the ~10 leftover blocking-clause $m=43$ branches still grinding on this host were retired once the census closed (their goal was achieved by a stronger method; the ledger already forbade grinding that approach). **In flight at write time**: the two `gold_*` break-free re-derivations of $n=13,14$ (restarted tonight with ~7 GB free after the branch cleanup); their status lands in PHASE_STATE when they finish. Artifacts: [`e18_sms_cell.py`](combinatorial/e18_sms_cell.py), `combinatorial/_cache/e18/sms_n*_m*.{json,cnf,log}`, the closed `enum_n16_m43.json`, and the AGREE record in `n16_compare.json`.

---

### L76. E18: L75's caveat (i) is now HALF-CLOSED by a second enumerator built on different technology (CDCL + blocking clauses, not canonical augmentation), and the E17 result reproduces on an independent host, toolchain and OS -- including, graph for graph, the 24 hardest instances. A second finding is a clean NEGATIVE: Gallai's 6-critical low-vertex structure, the "qualitatively stronger prune" L75 named as the way past the $n=17$ wall, is roughly two orders of magnitude too weak to matter.

**The scoping fact that governs the whole experiment** (and stops it being oversold): "the class is EMPTY at order $n$" is an $\exists$-statement over the $\binom{n}{2}$ edge variables, hence in NP, so a SAT UNSAT settles it independently; "every class member at order $n$ is 5-colorable" is $\exists\forall$, i.e. $\Sigma_2^p$, and is NOT SAT-encodable. E18 therefore attacks the enumeration half of E17 and leaves the colorability half where E17 has it.

**Replication on a second host (all of E17's gates).** This ran on the Windows laptop (32 cores, msys/ucrt64 gcc 15.2.0), not the Linux box of L75; it began with no venv, no packages and no nauty. After bring-up: core smoke 9/9, the $\chi\ge5$ anchor (Heule-826 UNSAT at $k=4$, 9.5 s), firewall 0 violations, and nauty 2.8.9 + `geng_hn` rebuilt ([`e17_build_geng_msys.sh`](combinatorial/e17_build_geng_msys.sh); needed `./configure` plus `-DAVOID_SYS_WAIT_H`). Gates (a)/(b)/(c) reproduce EXACTLY: 352/2001/15481 at $n=7/8/9$; $n=13,14$ windows empty; $n=15$ emits 11; the $m=48$ cell emits exactly the Shrikhande graph. At $n=16$, `geng_hn` returned **exactly 11,315**, and coloring all of them reproduced the verdict to the residue: **11,291 by DSATUR, 24 needing SAT, 0 hits**, and those 24 are **isomorphic to the 24 committed in [`e17_n16_sat_residues.json`](combinatorial/e17_n16_sat_residues.json), 24/24**. Two documentation gaps were closed: the 352/2001/15481 counts are the UNRESTRICTED mode (connected is 240/1546/13040, biconnected 68/508/4962; not recoverable from the numbers alone, now recorded), and the 11 graphs of the $n=15$ cell, which previously existed only in the Linux host's `_cache/`, are committed as [`e18_n15_class.json`](combinatorial/e18_n15_class.json) with exact $\chi$ per graph -- refining L75's "9 DSATUR + 2 SAT" into **nine $\chi=4$ and two $\chi=5$**.

**Stage 1, SAT emptiness ([`e18_sat_class.py`](combinatorial/e18_sat_class.py)), plus a NEW fact.** The default instance is deliberately a RELAXATION of E17's search (no connectivity, no degree cap, no edge window; only $K_4$-free + codegree $\le2$ + $\delta\ge5$), so UNSAT on it is strictly stronger than emptiness of E17's window and is independent of Kostochka-Yancey, covering the edge counts BELOW the KY floor that E17 never enumerated. Relaxation gate: 11/11 known $n=15$ members accepted. Result: UNSAT at $n = 9, 10, 11$ and **SAT at $n=12$** (a 5-regular member, 30 edges, confirmed by the independent both-free filter). **So the both-free class with $\delta\ge5$ first becomes nonempty at $n=12$** -- a question E17 never asked, since its $n\le12$ emptiness is about 6-CRITICAL candidates (via the KY floor exceeding the codegree ceiling), not about the class. Adding the KY floor back gives UNSAT at $n=13$ (0.13 s) and $n=14$ (1.43 s), an independent re-derivation of E17 gate (c) by CDCL rather than canonical augmentation.

**Stage 2, the Gallai negative ([`e18_gallai.py`](combinatorial/e18_gallai.py)).** L75 asked for "6-critical / Gallai-tree structure pushed INSIDE the generator"; this pushes it inside a CDCL search instead. In a 6-critical graph the subgraph induced by the degree-exactly-5 vertices is a Gallai forest, and $K_4$-freeness caps its blocks at $K_1/K_2/K_3$/odd cycles, giving two encodable consequences (the low subgraph has no $C_4$; each low vertex's low neighbourhood has max degree $\le1$). Calibration is non-circular -- instantiated at $k=4$ where real critical graphs exist, it must ACCEPT $W_5$, $C_5$, $C_7$, Moser and REJECT the cube $Q_3$: **5/5 PASS**. **Result: still SAT at $n=15,16,17,18$**, all under a second. Quantitatively, as a filter on the $n=15$ cell Gallai keeps 9 of 11, an **18% prune against the ~99% needed** to close L75's measured 100x gap at $n=17$. There is also a structural reason it cannot go inside geng as hoped: "low" means degree exactly 5 in the FINAL graph, and degrees only grow during augmentation, so the condition is sound only at $n=\text{maxn}$ -- a post-filter, which saves cheap coloring calls and not the generation that is the actual cost. **The $\Sigma_2$ colorability half, not the local structure, is what makes $n=17$ hard.**

**Stage 3, the second enumerator ([`e18_enumerate.py`](combinatorial/e18_enumerate.py)).** CDCL + blocking clauses, isomorph rejection outside the solver; pinning the edge count $m$ lets the blocking clause name only the positive edges (sound exactly because a distinct graph on the same $m$ cannot contain this one's edge set). **At $n=15$: 1,989 models $\to$ 11 classes, cell EXHAUSTED, 39.8 s, and all 11 match geng's 11 GRAPH FOR GRAPH (AGREE)**, with the per-$m$ split 9/2/0 also matching. At $n=16$ the SAT side FOUND all 11,315 classes ($10{,}583+645+75+10+1+1$, matching geng exactly), but formal exhaustion completed for FIVE of the six cells ($m=44..48$, $m=44$ via all 8 branches), leaving only $m=43$, which was PARKED rather than ground out after a measured wall: enumeration-by-blocking dies at ~$10^5$ blocking clauses per solver regardless of how the cell is split (8-way: 2 branches closed at 156k/543k models, 5 ran past 540k-760k without closing; 64-way: 4 closed at 32-42k, the rest crossed 100k and decayed from 50-70 to 8-23 models/s), so the fix is an enumerator that never emits duplicate labellings (SAT modulo symmetries), not more compute; the comparison on what is proved reads **matched 11,312, only-in-geng 3, only-in-SAT 0, property violations 0** (coverage rose 11,270 -> 11,312 as branches closed) -- i.e. CONSISTENT-PARTIAL: no contradiction anywhere the two methods overlap, with the shortfall being coverage from one parked cell, not disagreement. **Cost model (the transferable part): the binding cost is not the class count but the models-per-class ratio**, ~250-370 unsplit (the $m=43$ tail did not close in 8 h), dropping to **~14 when the cell is split 8 ways on the three highest-indexed vertex pairs** (a 17-25x reduction; splitting on the LOWEST pairs is useless because the lex-leader break already pins them -- at $n=15,m=41$ every surviving model landed in one branch). Branch balance is poor, and the blocking tail is an artefact of using lex-leader symmetry breaking at all: a genuine isomorph-free SAT generator (SAT modulo symmetries) is the obvious next tool.

**Architecture**: 1 (combinatorial). **Wrong-approach detectors**: PASS and inherited from E17 -- the class premise is Euclidean strict convexity (two unit circles meet in $\le2$ points, so codegree $\le2$); in $L^\infty$ codegree is unbounded and the premise dissolves; the $\mathbb{R}^1$ search space (mindeg 5 vs maxdeg 2) is empty; nothing lifts to a $\chi(\mathbb{Q}^2)\ge3$ claim. **No bound moved**, and no new $\chi\ge6$ candidate appeared at any order, which is what L75 predicts. **Honest scope**: this bounds the CLASS, not $\chi(\mathbb{R}^2)$; realizability (W3) is untouched; L75 caveat (ii) (the two counting lemmas are formalization-ready but not Lean-proved) is untouched. **What did not finish**: the two `gold_*` runs re-deciding $n=13,14$ with NO symmetry breaking (one was retired when it hit 2.15 GB and host free memory fell to 1.9 GB, endangering the enumeration branches), so those UNSATs currently rest on the sound-and-gated symmetry break rather than a break-free re-derivation; and 6/8 $m=43$ plus 1/8 $m=44$ branches. Both are cheap to finish on an idle machine. Full writeup [`e18_results.md`](combinatorial/e18_results.md); plan and gate ledger [`NIGHT_PLAN_2026-08-08.md`](orchestrator_sessions/NIGHT_PLAN_2026-08-08.md).

---

### L75. E17: the nauty door L69 left open is now UNBLOCKED AND WALKED THROUGH, and it closes with a theorem-grade negative: the both-free class ($K_4$-free AND $K_{2,3}$-free, the UDG-necessary class) contains NO $\chi\ge6$ member on $n\le16$ vertices, EXHAUSTIVELY, with a measured feasibility wall at $n=17$. The gcc host let us build nauty 2.8.9 plus a custom-pruned geng, `geng_hn` ([`e17_prune.c`](combinatorial/e17_prune.c) + [`e17_build_geng.sh`](combinatorial/e17_build_geng.sh)): incremental $K_4$ / $K_{2,3}$ rejection at the last-added vertex (sound and complete by subgraph-monotonicity along geng's augmentation order) plus a cherry-budget prune (final cherries $\ge\sum_v\binom{\max(\deg v,5)}{2}+10$ per future vertex, against the $K_{2,3}$ budget $2\binom{n}{2}$), plus two NEW sound search-space caps proved for the class: $\Delta\le(n-1)/2$ (codegree $\le2$ + mindeg 5 force $\sum_{w\sim a}(\deg w-1)\le2(n-1)$) and 2-connectivity (any $\chi\ge6$ member contains a 6-critical subgraph, itself in the class; 6-critical graphs are 2-connected with mindeg $\ge5$ and Kostochka-Yancey floor $m\ge(28n-18)/10$; the codegree ceiling $m\le n(1+\sqrt{8n-7})/4$ is automatic). So `geng_hn -C -d5 -D[(n-1)/2]` over the per-$n$ edge window (KY floor to codegree ceiling), then a 5-colorability filter, decides "does the class contain a $\chi\ge6$ member on $\le N$ vertices" once all $n\in[16,N]$ complete (Folkman floor, Lathrop-Radziszowski 2011: no $K_4$-free $\chi\ge6$ graph below $n=16$).

**Calibration (five gates, all PASS; the exhaustiveness claim rests on these).** (a) Plugin vs an INDEPENDENT pure-Python both-free filter ([`e17_bothfree_filter.py`](combinatorial/e17_bothfree_filter.py), disjoint code from the C plugin): exact count agreement 352 / 2001 / 15481 at $n=7/8/9$, and at $n=9$ the canonically-labeled output SETS are identical (labelg + diff). (b) The extremal $n=16$, $m=48$ cell emits EXACTLY ONE graph, the Shrikhande srg(16,6,2,2) (6-regular, all 120 codegrees $=2$, isomorphism-checked against the Cayley $\mathbb{Z}_4\times\mathbb{Z}_4$ construction), $\chi=4$, correctly rejected. (c) The Folkman floor holds: the $n=13,14$ windows contain ZERO both-free graphs at all; $n=15$ contains 11, all 5-colorable. (d) Core smoke test 9/9. (extra) The hit path was pre-validated end-to-end on $M^3(C_5)$ ($n=47$, $\chi=6$): DSATUR fail $\to$ in-process Cadical 5-UNSAT $\to$ provisional JSON $\to$ main-process portfolio confirm + valid 6-coloring.

**The main run and the verdict (2026-07-23).** The $n=16$ full window (43..48 edges; `geng_hn -C -d5 -D7`, 128 checkpointed res/mod parts on 16 cores) completed in 238,519 cpu-s (66.3 cpu-h, 4.4 h wall): **11,315 both-free candidates, every one 5-colorable** (11,291 by DSATUR instantly, 24 by in-process Cadical195), 0 independent-filter disagreements, 0 hits. With the per-$n$ sweep $n=13,14,15$ and the Folkman floor: **the smallest $K_4$-free $K_{2,3}$-free graph with $\chi\ge6$, if one exists, has $n\ge17$.** This upgrades the three-directional HEURISTIC negatives (greedy-up L65 $\to\chi4$, anneal-up L67 $\to\chi5$, repair-down L69 $\to$ stall) to an exhaustive verdict for $n\le16$, closing the "maybe the local searches just missed a small one" objection up to the feasibility wall (the heuristics probed $n$ up to 64, where the question stays heuristic-only), and it confirms L69's conjecture from the inside: at accessible $n$ the $K_{2,3}$ violations really are load-bearing for $\chi=6$.

**The wall (measured, not guessed).** Four sample parts of an $n=17$ mod=4096 split (`geng_hn -C -d5 -D8 -u 17 46:52`) each ran $>1{,}700$ cpu-s WITHOUT finishing, so $n=17$ costs $>80$ cpu-days on this host ($>5$ days on all 16 cores, and the true number is larger). The per-$n$ multiplier is consistent along the ladder: $n=15\to16$ was $\sim$100x ($2.4\times10^3\to2.39\times10^5$ cpu-s), $n=16\to17$ is $>29$x (58 cpu-s per equivalent $n=16$ slice vs $>1{,}700$ unfinished), so $n=18$ extrapolates to thousands of cpu-days. The geng-enumeration route saturates at $n=16$ here; $n=17$ needs either $\sim$100x compute (a hundred cores for a week) or a qualitatively stronger prune (6-critical / Gallai-tree structure pushed INSIDE the generator, clique-cover lower bounds, or a canonical-form generator specialized to near-extremal codegree-2 graphs).

**Architecture**: 1 (combinatorial). **Wrong-approach detectors**: PASS, and central: the class premise is Euclidean strict-convexity rigidity (two unit circles meet in $\le2$ points). In $L^\infty$ two unit spheres can share a segment, codegree is unbounded, and the premise dissolves (an $L^\infty$ analog of E17 does not exist, as required); the $\mathbb{R}^1$ search space (mindeg 5 vs max degree 2) is empty; nothing lifts to a $\chi(\mathbb{Q}^2)\ge3$ claim. E17 makes no $\chi(\mathbb{R}^2)$ claim at all: a hit would be an abstract candidate whose UDG realizability is the separate, explicitly-open step (the L63/L74 rigidity wall). **Honest scope**: this bounds the CLASS, not $\chi(\mathbb{R}^2)$, and the codegree-ceiling / KY-floor asymptotics ($\sim0.7n^{1.5}$ vs $2.8n$) still guarantee the class is nonempty of 6-critical candidates at large $n$: the object may simply live beyond enumeration reach. VERIFIER and ADVERSARY passes are now COMPLETE and GREEN. VERIFIER ([`e17_verification.md`](combinatorial/e17_verification.md)): overall VERIFIED, 4 of 5 targets VERIFIED plus 1 VERIFIED-WITH-CAVEAT, zero blocking findings (independent recount of 352/2001/15481 at $n=7/8/9$ exact; the $n=15$ cell and 9/9 sampled $n=16$ parts reproduce exactly; both counting lemmas re-derived constant-by-constant with no off-by-one, the strict `>` in the cherry budget confirmed load-bearing (equality attained by Shrikhande); caveat: the 24 SAT-residue graphs were not persisted). ADVERSARY ([`e17_adversary.md`](combinatorial/e17_adversary.md)): overall PASS, all five attack surfaces SOUND (detector self-assessment SOUND-WITH-CAVEAT, since $\mathbb{Q}^2$/$L^\infty$/$\mathbb{R}^1$ are partly inapplicable to a non-existence theorem; the operative safeguard, both-free necessity for $\mathbb{R}^2$ UDGs, verified); PRUNE/PREPRUNE fuzzed 272,589 fires with 0 valid completions cut, and it closed the VERIFIER's residue-persistence caveat by re-deriving all 24 SAT residues (0 checkpoint mismatches, 0 disagreements across glucose42/minisat22/cadical/greedy). Post-pass: the 24 residues are now the tracked artifact [`e17_n16_sat_residues.json`](combinatorial/e17_n16_sat_residues.json) and [`e17_nauty_host_search.py`](combinatorial/e17_nauty_host_search.py) was patched to persist `sat_residue_g6` going forward. Two caveats to carry verbatim into any future fold: (i) enumeration completeness at $n=15,16$ rests on geng plus the verified prune lemmas, no independent second enumerator (same epistemic position as the Folkman-number literature); (ii) the two counting lemmas are formalization-ready (Mathlib `SimpleGraph`/`Finset` double counting) but not yet Lean-proved. Artifacts: [`e17_nauty_host_search.py`](combinatorial/e17_nauty_host_search.py) (driver), [`e17_window_table.py`](combinatorial/e17_window_table.py), `~/.local/bin/geng_hn` (rebuildable via [`e17_build_geng.sh`](combinatorial/e17_build_geng.sh)), run log + checkpoint under `combinatorial/_cache/e17/`.

---

### L74. The gradient thread SCALED TO GPU (RTX 5090): the L73 flagship negative sharpens from "one run $\to\chi=3$" to "$\sim$221k parallel adversarial runs $\to\chi=3$, never even $\chi=4$," and the L63 rigidity wall is re-confirmed under 16k starts per host. New module [`gradient/gpu/`](gradient/gpu/): a batched, device-parameterized `(B,n,2)` core (float32 GPU search $\to$ float64 CPU refine, the firewall intact), validated against the same calibration answers as `diff_udg` (Moser + triangular patch realize LEGAL to $\sim10^{-10}$; $K_4$ refuses; oracle reproduces triangle `@k=2=0.333` and $K_5$ `@k=4=0.1`). **No bound moved.** Env facts settled: the 5090 (sm_120) needs `torch==2.11.0+cu128` (the cu128 index has no 2.12 build); `python-sat` was silently absent (so `sat_chi` returned `None`), now installed and the core smoke gate passes 9/9.

**Campaign alpha -- realizer sweep at scale (a stronger L63 corroboration).** Multi-start legal-UDG realizer, **16,384 embeddings per host** (B=4096 $\times$ 4 seeds), over all 8 E13/E13b hosts PLUS the $\chi=5$ in-class graph e15b_chi5_n29. EVERY host refuses to realize (best `max_edge_err` $0.35$-$0.71$, never closing to the $<10^{-6}$ legality bar) and is rigidity over-determined by $+43$ to $+51$ edges. The continuous / rigidity corroboration of L63's codegree wall, now under $10^4$ starts rather than one (cf. L73 follow-up 1's single-start version). The $n=29$ $\chi=5$ graph is the prize case: realizing it would be a new $\chi=5$ UDG (the most-leveraged move), but it is over-determined by $+43$ and refuses. `gradient/gpu/_cache/alpha_results.json`.

**Campaign beta -- adversarial generator at scale (the sharpened flagship negative).** The GD-native Arch-1 route ($\max_{\text{coords}}\min_{\text{coloring}}$ at $k=4$), scaled to **221,184 parallel GDA runs** (54 configs: $n\in\{9,12,15,18,21,24\}$, target-deg $\in\{3,4,5\}$, $\tau\in\{0.05,0.08,0.12\}$, B=4096 each), with a below-target degree hinge + crispness reward forcing dense near-unit construction. The 96 densest/hardest survivors were legalized and SAT-certified. **Result: $\chi$ histogram $=\{3:96\}$ -- every one is $\chi=3$, not one reached even $\chi=4$.** Densest built: $n=24$, $\lvert E\rvert=77$; densest that legalized cleanly: $n=24$, $\lvert E\rvert=76$, $\chi=3$, gap $0.051$. Only $4/96$ densest emergent topologies legalized into clean UDGs (the other 92 are over-determined near-unit clouds -- the same wall alpha hits, and the L73-follow-up-2 near-unit-non-edge faithfulness trap); all 96, legal or not, are $\chi=3$. Reading: massive multi-start unit-distance construction collapses to triangular-lattice-like $\chi=3$ patches; maximizing unit-edge count finds the densest packing (the lattice, $\chi=3$), and $k=4$ colorability pressure does not lift it even to $\chi=4$. The $\chi=4$ Moser rigidity (two rhombi at an irrational rotation) is a geometrically ISOLATED target smooth pressure does not discover.

**Architecture**: 1 (combinatorial, via the continuous surface). **Wrong-approach detectors**: CENTRAL -- the objective is coordinate-grounded (real Euclidean unit distances), so it cannot lift to $\mathbb{Q}^2$ ($\chi=2$); a pure abstract-adjacency soft-coloring objective is the excluded mistake. **Honest scope.** Beta's $\chi=3$ monolith is a property of THIS objective (density + soft-coloring + spread), not a theorem that GD cannot build $\chi=4$; a Moser-seeded start or a different edge model might. The claim is bounded: of 96 densest/hardest survivors up to $\lvert E\rvert=76$, all $\chi=3$; no $\chi\ge5$ candidate flagged. Full writeup [`gradient/gpu/RESULTS.md`](gradient/gpu/RESULTS.md); gate `python -m experiments.gradient.gpu.validate`.

**Follow-up -- campaign gamma, beta SEEDED from the $\chi\ge4$ lineage ([`campaign_gamma.py`](gradient/gpu/campaign_gamma.py)), resolving the beta question: the $\chi=3$ monolith is a REACHABILITY wall, not a representational one.** Three experiments. (E1 representation) Extracting the unit-distance graph from the true Moser and P510 embeddings and SAT-ing recovers $\chi=4$ ($\lvert E\rvert=11$) and $\chi=5$ ($\lvert E\rvert=3353$ at tol 0.02, the L73-follow-up-2 thickening, $\chi$ unchanged) -- the verify pipeline represents $\chi\ge4$ fine, so beta's monolith is NOT a pipeline artifact. (E2 preservation) Seeding the adversarial GDA AT the Moser spindle (B=2048 jittered copies $\times$ 3 seeds) and running the full outer pressure: **every seed goes $2048/2048\to\chi=4$ post-pressure** at median non-edge gap $0.263$ (Moser's own gap -- clean intact spindles); the pressure does not relax $\chi=4$ to $\chi=3$, it REPAIRS jittered-down copies back to $\chi=4$. So GD HOLDS $\chi=4$ when it starts there; it just cannot FIND it from a random cloud (the $\chi=4$ Moser rigidity is geometrically isolated). (E3 scaffold growth) Seeding a Moser core + $(n-7)$ free vertices ($n\in\{10,12,15,18\}$, 24 configs, B=4096) and letting the adversary grow it: of 96 densest survivors **$\{\chi{=}3:79,\ \chi{=}4:17\}$, best legal $\chi=4$, never $\chi=5$** (25 legalized cleanly: $\{\chi{=}3:20,\chi{=}4:5\}$, densest legal grown $\chi=4$ at $n=18,\lvert E\rvert=58$). Growing a $\chi=4$ core under density/colorability pressure does not climb to $\chi=5$ and usually DILUTES the rigidity back to $\chi=3$ lattice as it densifies. **The clean statement: the continuous surface can REPRESENT and PRESERVE $\chi=4$, but neither builds it from scratch (beta) nor grows it past $\chi=4$ (gamma E3) -- GD holds hardness but cannot manufacture it, exactly the local-one-sided governing fact.** Next open lever (unchanged, now sharper): the fixed-edge-set + explicit candidate-new-edge model (L73 Option-A), which is the only edge model that could add a UNIT edge to a hard core without diluting it into lattice; the global soft-adjacency cannot.

**Follow-up -- campaign delta, the fixed-skeleton + candidate-edge model ([`campaign_delta.py`](gradient/gpu/campaign_delta.py)) BUILT: it solves the dilution problem exactly, and isolates the real ceiling.** The model L74 named as the open lever, and de Grey's actual construction principle: PIN a rigid $\chi=4$ core (Moser, $\lvert E\rvert=11=2n-3$, isostatic, so its SHAPE cannot dissolve once its edges hold at unit), attach $m$ FREE new vertices, and pull each toward unit distance from vertices it currently SHARES A COLOR with (the differentiable proxy for a hardening edge). Every unit pair is intended (core or accepted candidate), so it is a legal UDG by construction -- no faithfulness trap, unlike the global soft-adjacency. **Result (B=4096 per core size, $m\in\{2,\dots,12\}$, $n=9$-$19$): $4096/4096$ runs HELD the core at EVERY size (96/96 verified `core_ok`), adding up to $+19$ hardening unit edges -- dilution solved completely (contrast gamma E3, which dissolved 79/96 grown configs to $\chi=3$; delta dissolves ZERO).** So one CAN pin a rigid $\chi\ge4$ core and grow unit edges onto it without dissolving it. **But $\chi$ stays exactly 4** (histogram $\{4:96\}$, uniform over all $m$). This is not a method failure but the real ceiling made explicit: the smallest KNOWN $\chi=5$ UDG is $\sim$500 vertices (Polymath16), and no small $\chi=5$ UDG is known to exist, so growth to $n\le19$ cannot reach $\chi=5$ -- the target does not exist at that size. **Net conclusion for the continuous surface: the realizability-preserving growth primitive now exists and works (100% core-hold), but the $\chi=5$ object lives at $\sim$500-vertex scale, which undirected random growth will not reconstruct; the productive path to $\chi\ge5$ stays SAT-based construction (de Grey / Polymath lineage), with delta available as the continuous realizability gate for any grown candidate.** Full writeup [`gradient/gpu/RESULTS.md`](gradient/gpu/RESULTS.md).

---

### L73. A GRADIENT-DESCENT attack surface is built, calibrated, and firewalled: the new [`gradient/`](gradient/) thread maps exactly where continuous optimization can bite on $\chi(\mathbb{R}^2)$ and where it only pretends to. The governing constraint, made explicit: GD is LOCAL and ONE-SIDED, so it produces upper bounds / constructions (existence of a coloring or embedding) but NEVER a $\chi\ge6$ lower bound, which needs a SAT/dual certificate. Two surfaces are legitimate (construction + convex-dual); a third (pure soft-coloring) is structurally wrong because it lifts to $\mathbb{Q}^2$ ($\chi=2$) and is excluded by design. No bound moved; this is runnable infrastructure with every hard claim routed back to SAT.

**Four options, three built and calibrated, one spec'd.** Shared torch core [`gradient/diff_udg.py`](gradient/diff_udg.py): `edge_residual_loss`, `nonedge_margin_loss` (the non-edge term the legacy realizer omits), `soft_adjacency` ($a_{ij}=\exp(-(d_{ij}-1)^2/2\tau^2)$, the graph as a smooth function of coordinates), `coloring_loss` (softmax monochromatic weight), `spread_penalty`. (A) **Adversarial coordinate$\leftrightarrow$coloring co-optimization** [`gradient/adversarial.py`](gradient/adversarial.py): $\max_{\text{coords}}\min_{\text{coloring}}$ colorability, the GD form of the live Arch-1 route; CANNOT lift to $\mathbb{Q}^2$ because driven by real unit distances. (B) **Legal-UDG realizer** [`gradient/realize.py`](gradient/realize.py): the patched [`realizability_w3_clamp.py`](combinatorial/realizability_w3_clamp.py) -- adds the non-edge margin so a residual-zero drawing is a LEGAL UDG of the intended graph, not one with accidental extra unit edges. (C) **First-order spectral push**: spec'd against [`fractional/`](fractional/) (Burer-Monteiro Lovász $\vartheta$ on the sphere = the one surface whose dual CERTIFIES a lower bound), not duplicated; L72 closed order-2 measurable, the open lever is a richer basis / $SE(2)$. (D) **Flexible-but-color-rigid gadget search** [`gradient/gadget_search.py`](gradient/gadget_search.py): the [`rigidity_w3_escape.py`](combinatorial/rigidity_w3_escape.py) kill-test made runnable -- flex side = rank test on the rigidity matrix with $\nabla\lvert p_s-p_t\rvert^2$ appended; clamp side = SAT (same/diff/free).

**Calibration (load-bearing; a negative is then structural, not solver noise).** Inner colorability oracle reproduces known $\chi$ EXACTLY: unit triangle `loss@k=2 = 0.333` (one of three edges forced monochromatic), `@k=3 ~ 1e-4`; $K_5$ `loss@k=4 = 0.1`, `@k=5 ~ 4e-5`. Realizer (B): Moser spindle + triangular patch return LEGAL UDGs at `max_edge_err ~ 1e-16`; $K_4$ (needs $\mathbb{R}^3$) correctly FAILS. Gadget (D): flexible clamps EXIST at low $k$ (unit path forces a same-class pair SAME at $k=2$ and still flexes; rhombus likewise) but VANISH far below $k=5$; pentagon and Moser have none -- the kill-test cell (flex AND clamp@5) stays empty for every small gadget, reproducing the structural reality.

**The flagship's honest negative (A).** Naive alternating GDA from a RANDOM seed, with the degree-budget term forcing edge formation, builds a real 46-edge near-unit graph at the target density (mean soft-degree ~3.1) but the inner colorer always wins: the discovered graph is only $\chi=3$. A single GDA run does NOT discover a hard-to-color UDG (that IS the open problem). This is the finding, and it points at the levers: seed from the $\chi=5$ lineage (P510) not random, stronger outer optimization (annealing/restarts), or compose the Option-D gadgets. The degree term is essential -- without an edge-formation incentive the adversary's trivial optimum is "no unit distances at all" (an empty graph is 1-colorable), which the first run hit (1 edge, $\chi=2$).

**Architecture**: cross-cutting (1 combinatorial via A/B/D, 2/3 measurable/fractional via C). **Wrong-approach detectors**: CENTRAL here, not N/A -- the soft-coloring-only objective is the canonical $\mathbb{Q}^2$-liftable mistake and is excluded; A is built to route chromatic pressure through real Euclidean unit distances. Scripts run green: `python -m experiments.gradient.{realize,gadget_search,adversarial}`.

**Follow-up 1 -- B applied to the L63 hosts ([`realize_hosts.py`](gradient/realize_hosts.py)), a continuous corroboration of the codegree wall.** Pointed the legal-UDG realizer at all 8 manufactured $K_4$-free 6-chromatic hosts (E13/E13b, $n=18$-$20$). Calibration passes (Moser LEGAL, $2.2\times10^{-16}$). EVERY host FAILS to realize as a legal UDG (`max_edge_err` $\sim0.5$, never closing) and is rigidity OVER-determined by $+43$ to $+51$ edges ($\lvert E\rvert-(2\lvert V\rvert-3)\gg0$, e.g. shrunk_18_from_M2_W5: $79$ edges vs $33$ DOF). This is L63's codegree wall seen from the CONTINUOUS / rigidity side, fully independent of SAT: the plane geometry alone refuses these graphs, no $K_{2,3}$-freeness argument invoked. A host that realized legally would be a live $\chi\ge6$ candidate; none does, consistent with L63.

**Follow-up 2 -- A seeded from P510 ([`adversarial_p510.py`](gradient/adversarial_p510.py)), a negative plus a relaxation-limit finding.** Seeded the adversary at the TRUE coords of P510 ($510$ vtx, $2504$ unit edges, $\chi=5$) instead of a random cloud. GDA does NOT push toward $\chi\ge6$: at a loose $\tau$ it RELAXES density toward an easier 5-coloring (mean soft-degree $28\to11$, loss $0.10\to0.011$, deformed graph SAT-confirmed 5-colorable); at a faithful $\tau$ it is gradient-STARVED (loss flat $\sim0.079\to0.081$ over 200 steps, density pinned at target). A STRUCTURAL caveat surfaced from the $\tau$ sweep: P510 has hundreds of NON-edge pairs near unit distance ($6683$ soft edges at $\tau{=}0.05$, still $3110$ at $\tau{=}0.012$, vs $2504$ true), so the global `soft_adjacency` cannot be made faithful to a dense real UDG without killing the gradient. Consequence: Option A's soft-adjacency model suits SPARSE / synthetic configs; a dense real-UDG seed needs a different edge model (optimize over the fixed true edge set plus an explicit candidate-new-edge mechanism, not a global softmax over all $\binom{n}{2}$ pairs). The honest headline holds: a single GDA pass does not crack the open problem from either seed.

Next: C as a `fractional/` increment (first-order Lovász $\vartheta$); for A, the fixed-edge-set + candidate-edge model on a sparse $\chi=5$ seed; B is ready as the realizability gate for any future in-class host candidate.
...
**The monotonicity gate (why $k=4$ alone closes the route).** A feasible order-2 moment solution at $k=4$ extends to a feasible one at $k=5$: zero out all color-5 moments (no vertex uses the 5th color); NORM still sums to 1 over the four used colors, and the moment matrix gains only zero rows/columns, preserving PSD; the $S_k$-symmetrization is lossless (L44), so the symmetric $k=5$ relaxation is feasible too. Hence $k=4$ feasible $\Rightarrow$ $k=5$ feasible $\Rightarrow$ order-2 cannot certify $\chi_m\ge6$. Running $k=5$ (the same reduced size, L71) would be provably futile, so it was not run.

**Why the negative is consistent (not a solver artifact).** Order-2 carries the inclusion-exclusion CONGRUENCE constraints up to subset size 4 (L41/L47), but the single-class IE-LP needed IEC up to size 5 to cross $1/4$ and certify $\chi_m\ge5$ (L36). The order-2 PSD coupling was the hope that it might compensate for the missing size-5 reach; this run shows it does NOT. So the measurable SDP hierarchy needs at least order 3 (IEC size 6) to have a chance at $\chi_m\ge5$ from the primal+PSD side, let alone $\chi_m\ge6$.

**Architecture**: 2/3 (measurable / fractional). The capstone of the L70/L71 build: the matrix-free operator (L70) + solver (L71) were constructed and validated precisely to make this $X_{23}$-scale run possible, and the run delivered a verdict. Script [`e3u_order2_solver.py`](fractional/e3u_order2_solver.py) (`run_x23`), verdict `fractional/e3u_x23_k4_verdict.json`, checkpoints in `_cache/`.

**Honest scope / caveats.** No bound moved (the outcome is a NEGATIVE: order-2 is too weak). (i) First-order (linearized-ADMM) verdict, validated 3/3 against e3q's dense margins on small configs (rhombus, Moser $k=3,4$); there is no independent $X_{23}$-scale cross-check because the dense affine map is the $\sim$195 GiB wall the solver was built to avoid. (ii) "Feasible" is the ABSENCE of a certificate, so there is no certificate object to externally verify -- but the negative is exactly "order-2 does not help," and the margin's 600x monotone collapse with no plateau is a strong, clean feasible signal. (iii) The run was stopped at iter 5120 (margin $0.0021$) before the formal $\text{prim}<10^{-3}$ crossing; the trajectory (monotone, no plateau over 5120 iters) made the conclusion certain. **Wrong-approach detectors**: N/A ($O(2)$-averaged Euclidean SDP; the e3i/e3j 1D control guards the LP lineage). Next: order-3 (IEC size 6) is the formal continuation but its moment matrix is far larger (a much harder build, likely impractical even matrix-free); the more honest redirect is that the measurable SDP hierarchy is not a near-term route to $\chi_m\ge6$, which throws weight back onto the Architecture-1 missing object (a rigid $\ge5$-chromatic / $\ge6$-chromatic UDG, L33).

---

### L71. The matrix-free order-2 measurable SDP SOLVER is BUILT and VALIDATED, turning L70's blocked prerequisite into a running solve, plus a structural bonus: $X_{23}$ order-2 at $k=5$ (the $\chi_m\ge6$ frontier) is the SAME reduced size as $k=4$ (the $\chi_m\ge5$ validation), so the frontier costs no more than the check. [`e3u_order2_solver.py`](fractional/e3u_order2_solver.py): a LINEARIZED (proximal) ADMM that wraps the e3s operator. The only hard cone is the PSD blocks; it splits them off as a consensus $P_b=\mathrm{apply\_A}(y)$, takes one gradient step through $\mathrm{apply\_AT}$ (so the quadratic coupling never needs an inner solve), and the remaining x-update is a SPARSE QP with NO psd cone (NORM/MARG/Bochner + nonneg + the $L_1$ slack), re-solved each iteration via a parametrized cvxpy program. The PSD step is 4 eigendecompositions (max side 735); the operator forms only the $D\times D$ moment matrix (~125 MB at $k=4$, ~308 MB at $k=5$), never the ~195 GiB dense affine map (L70). So the RAM wall is gone.

**Validation (the load-bearing gate, PASS 3/3).** Against e3q's DENSE order-2 reference on small congruence-reduced configs, agreeing on the feasible/certify verdict: rhombus $k=4$ (e3q feasible) -> primal residual closes to $10^{-7}$ (FEASIBLE); Moser $k=3$ (e3q certifies $\chi\ge4$) -> primal residual PLATEAUS at $0.278$ (CERTIFIES); Moser $k=4$ (e3q feasible) -> residual $\to5.6\times10^{-6}$ (FEASIBLE). The mechanism is correct and clarified a subtlety: e3q's certificate is INFEASIBILITY of $\{\text{PSD}+\text{NORM}+\text{MARG}\}$ (status `infeasible`, not a positive slack-margin), and the matrix-free analogue is exactly "the ADMM primal residual cannot be driven to 0" -- feasible $\Leftrightarrow$ residual $\to0$, certifying $\Leftrightarrow$ residual plateaus above a feasibility threshold ($\sim10^{-3}$).

**The $k=4\equiv k=5$ size collapse (measured, e3t).** Both $k=4$ and $k=5$ on $X_{23}$ reduce to $n_{\text{orb}}=48342$ moment variables, 4 PSD blocks, max block 735, ~195 GiB dense map -- IDENTICAL (only $D$ differs, 3953 vs 6206). Reason: the $O(2)$-congruence classes of colored subsets of size $\le4$ SATURATE at $k=4$ (a 4-subset realizes at most 4 distinct colors), so $k=5$ adds no new classes. Consequence: running the $\chi_m\ge6$ frontier ($k=5$) is no more expensive than the $\chi_m\ge5$ validation ($k=4$); whatever solves one solves the other.

**Architecture**: 2/3 (measurable / fractional). Scripts [`e3u_order2_solver.py`](fractional/e3u_order2_solver.py) (solver + validate + `run_x23`), [`e3t_x23_order2_run.py`](fractional/e3t_x23_order2_run.py) (size measurement); result caches in `_cache/`.

**Honest scope / status.** No bound moved yet. The solver is VALIDATED on small configs and DEMONSTRATED RUNNING on $X_{23}$ $k=4$: operator+linear-system build 264 s, then iterating with the primal residual decreasing ($5.34\to1.91$ in 20 iters), the 195 GiB RAM wall gone. Two implementation facts settled. (a) OSQP-DIRECT x-update is MANDATORY, not just an optimization: the cvxpy x-update OOMs at $X_{23}$ scale (DPP canonicalization of a 48342-variable proximal QP materializes a ~35 GiB dense quadratic-coefficient array). The replacement assembles the QP's constant $P,A$ once and OSQP factorizes once, updating only the linear term $q$ per iteration (warm-started); re-validated 3/3 against e3q with no verdict change, ~2x faster. (b) Convergence is slow: $\|\mathrm{apply\_A}\|^2\approx4801$ on $X_{23}$ forces a tiny proximal step $\tau=0.9/L\approx1.9\times10^{-4}$, so the linearized ADMM needs many thousands of iterations at $\sim3$-$6$ s each $=$ HOURS to a verdict; the solve is launched DETACHED (survives the session, checkpoint `_cache/e3u_x23_k4_ckpt.json`). The verdict (FEASIBLE $=$ order-2's IEC-size-4 reach insufficient even for $\ge5$, redirect to order-3; or CERTIFY $=$ the lift works, then run $k=5$ for the $\ge6$ frontier at the same cost) is pending that run. Order-2's payoff stays speculative (L50: no order-2-vs-order-1 separation below $X_{23}$ scale). **Wrong-approach detectors**: N/A (solver on the $O(2)$-averaged Euclidean SDP; the e3i/e3j 1D control already guards the LP lineage). Next: read the $X_{23}$ $k=4$ verdict from the checkpoint; acceleration (over-relaxation / adaptive $\rho$ / a larger $\tau$ than the conservative bound) would cut the iteration count if the run is too slow.

---

### L70. The order-2 measurable SDP blocker (the standing $\chi_m\ge6$ prerequisite of L48/L50) is now PRECISELY characterized, and BOTH hoped-for shortcuts are ruled out: $X_{23}$ has TRIVIAL geometric symmetry and the $S_k$-reduced order-2 affine map is 100% DENSE. So the order-2 constraint data at $X_{23}$ is irreducibly $\sim$195 GiB if materialized, and the only viable backend is MATRIX-FREE (operator form), not the sparse-matrix assembly one might hope for. This refines the L48/L50 spec and is a build-path verdict, not a bound. Probe [`e3r_order2_sparsity_probe.py`](fractional/e3r_order2_sparsity_probe.py); result `_cache/e3r_order2_sparsity_probe.json`.

**The two levers, both measured dead.** (1) EXTRA SYMMETRY -- the de Laat-Vallentin reductions are powerful precisely when a large symmetry group collapses the SDP (sphere packing, equiangular lines). $X_{23}$, found by Ambrus's beam search, has **$\|\mathrm{Aut}(X_{23})\|=1$** (23 singleton vertex orbits, computed exactly as the automorphisms of the complete graph on 23 nodes with edges labelled by the 27 exact squared-distance classes). There is NO geometric symmetry beyond the $S_k$ color relabeling, so the PSD block size ($\sim$735) is fixed by $S_k$ alone and cannot shrink. The very feature that makes $X_{23}$ a good $m_1$ certificate (asymmetry, 27 distinct distances) is what denies the SDP its symmetry collapse. (2) SPARSITY -- the $S_k$-block-diagonalized affine map $T$ (per block, shape $\text{mult}^2\times n_{\text{orb}}$) is **100% DENSE** on every block measured (Moser $n=7$, double-Moser $n=10$, both $S_k$ and $O(2)$-congruence-reduced: $988604/988604$, $30310400/30310400$, etc.), because each coefficient $C_o=F_s^\top E_o F_t$ projects a sparse orbit indicator $E_o$ through the DENSE symmetry-adapted basis $F$ (Murota randomized block-diagonalization) and comes out full. So a `scipy.sparse` assembly does not help: at $X_{23}$ scale it is $\sim$292 GiB (CSC index overhead) vs the $\sim$195 GiB dense largest block.

**Consequence (the sharpened spec).** Order-2 $X_{23}$ ($n_{\text{orb}}\sim48342$ congruence-reduced, largest block $\sim735$) has $\sim$195 GiB of affine-map data in the largest block alone, dense and unsymmetrizable beyond $S_k$. A solver that MATERIALIZES the constraint matrix (cvxpy canonicalization, or any sparse-matrix backend) cannot run it. The build is therefore a MATRIX-FREE first-order conic solver: recompute the operator applications $A y$ and $A^\top z$ on the fly each iteration from $F$ + the orbit incidence, trading the 195 GiB of RAM for per-iteration einsum recompute. That is the precise (and heavier) form of the L48/L50 "custom sparse conic backend."

**Honest scope / what it redirects.** No bound moved; a diagnosis. Combined with L50 (order-2 NEVER strictly separates from order-1 on any runnable config $\le n=10$; the order-2 advantage, if any, only appears at $X_{23}$ scale), building the matrix-free backend is BOTH weeks of work AND speculative: order-2's value at $X_{23}$ is unknown until it runs, and the per-iteration einsum cost (largest block $\sim48342\cdot735^2\cdot d$ FLOPs) makes even a matrix-free solve heavy (hours-to-days per Phase-1). The measurable $\chi_m\ge6$ thread thus rests on a high-cost, uncertain-payoff engineering build with no cheaper test available (the L50 "no shortcut" conclusion is reconfirmed and made precise). **Wrong-approach detectors**: N/A (solver-scaling diagnosis on the $O(2)$-averaged Euclidean SDP; no bound, $\mathbb{Q}^2$ legitimately exempt for the measurable thread). Architecture: 2/3 (measurable / fractional).

**Addendum (the matrix-free operator BUILT + validated, the backend's correctness-critical core).** The first increment of the L70 backend is done: [`e3s_order2_operator.py`](fractional/e3s_order2_operator.py) computes the order-2 affine map MATRIX-FREE, never allocating the dense $(\text{mult}^2, n_{\text{orb}})$ array. FORWARD $A: y\mapsto\{C_{\text{block}}(y)\}$ forms the $D\times D$ moment matrix $M(y)$ from the orbit incidence and projects each block as $C_{\text{block}}(y)=\frac1d\sum_l P_l M(y) P_l^\top$ ($P_l=$ the $l$-th column-slice of the block's stacked copy bases); ADJOINT $A^\top:\{Z_{\text{block}}\}\mapsto\nabla_y$ forms one $D\times D$ intermediate $W=\sum_{\text{block}}\frac1d\sum_l P_l^\top Z P_l$ and reduces $(A^\top Z)_o=\sum_{(a,b)\in\text{orbit }o}W[a,b]$. VALIDATED against e3q's dense $(C_{\text{const}}, T)$ reference on triangle/rhombus/Moser, $S_k$ and $O(2)$-congruence: forward matches to $\le1.4\times10^{-13}$, the adjoint identity $\langle Ay,Z\rangle=\langle y,A^\top Z\rangle$ to $\le1.4\times10^{-13}$ (a wrong operator would fake any downstream certificate, so this gate is load-bearing). STORAGE: the matrix-free footprint is $O(D^2)$ -- on $X_{23}$ $\sim$294 MB (moment matrix) + $\sim$294 MB (basis) vs the $\sim$195 GiB dense largest block, $\sim$680x, fits in RAM. So the 195 GiB RAM wall is REMOVED; what remains to run $X_{23}$ order-2 is to wrap this operator in a first-order conic solver (matrix-free PSD projection per block + the linear NORM/MARG/Bochner/IEC constraints + Phase-1 objective), the next increment. Per-iteration cost is the operator application (a handful of $(\text{mult}\times D)(D\times D)$ matmuls per block), feasible. Order-2's payoff at $X_{23}$ is still unknown until that solve runs (L50). Result `_cache/e3s_order2_operator.json`.

Next, if pursued: wrap e3s in a matrix-free CLARABEL/SCS-style first-order solver and run $X_{23}$ $k=4$ (retest $\ge5$) then $k=5$ (open $\ge6$); OR a search for a SYMMETRIC config that crosses $1/4$ (so the de Laat-Vallentin symmetry collapse applies and order-2 becomes cheap) -- the latter sidesteps the wall but is itself the hard problem $X_{23}$'s asymmetry solved.

---

### L69. E16 (= E15-v2, the TOP-DOWN repair direction): starting from a graph that already HAS $\chi=6$ and locally repairing its violations of the UDG-necessary class, holding $\chi\ge6$ as a hard constraint, drives the codegree excess down 66% but GENUINELY STALLS far from in-class, never reaching $\chi=6$ K_{2,3}-freeness. This completes a three-direction heuristic negative on the in-class $\chi\ge6$ object (greedy-up L65 $\to\chi4$; anneal-up L67 $\to\chi5$; repair-down here $\to$ stalls), and surfaces the real content: in $M^3(C_5)$'s neighborhood of graph-space the codegree ($K_{2,3}$) violations are LOAD-BEARING for $\chi=6$ -- you cannot shed the excess without dropping the chromatic obstruction. This is L63's codegree wall seen from the INSIDE. Seed $M^3(C_5)$ ($n=47$, triangle-free 6-CRITICAL, so $K_4$-free for free; $m=236$, **434 codegree-violating pairs, excess 1068, max codegree 11**, right at the $n=47$ ceiling). Verified floor (independent Maple+Cadical+Glucose portfolio): the canonical repair (codegree-safe moves only) reaches **$n=47$, $m=164$, $K_4$-free, $\chi=6$ (5-UNSAT and 6-SAT both confirmed), but still 166 violating pairs / excess 359 / max codegree 10** (the sparsify follow-up below then strips this to the $m=131$ critical floor at the SAME excess; that $m=131$ graph is the committed artifact `e16_repair_floor.json`). So repair achieves $m\,236\to164$ ($-31\%$), excess $1068\to359$ ($-66\%$), violating pairs $434\to166$ ($-62\%$), then stalls.

**Architecture**: 1 (combinatorial). Script [`e16_topdown_repair.py`](combinatorial/e16_topdown_repair.py) (CPython; in-process symbreak `solve_color` for the decisive 5-colorability check, pure-Python Kempe-walk coloring sampler), verifier [`e16_verify_floor.py`](combinatorial/e16_verify_floor.py), committed floor `combinatorial/e16_repair_floor.json`; caches/distribution under `_cache/e16_topdown_repair/` (`AGGREGATE.json`, gitignored).

**The criticality trap and the escape (the method).** A 6-critical seed has NO removable edge (deleting any drops $\chi$ to 5, L61), so pure violation-removal is vacuous. The escape is a PAIRED REWIRE: remove a codegree-violating edge $e_1=(a,b)$ ($\chi$ drops to 5; $a,b$ are forced-same in $G-e_1$ by criticality), then add a DIFFERENT codegree-safe + $K_4$-safe edge $(c,d)$ that is ALSO forced-same in $G-e_1$ (found by the L62 sampling filter: monochromatic across many Kempe-walk 5-colorings, confirmed by one 5-UNSAT check), which restores $\chi$ to exactly 6. Net excess strictly DOWN. Once rewires break criticality, pure removes cascade (sparsify $m$ and excess together). Every accepted move lowers excess and holds $\chi=6$, so the descent is monotone; the only question is the floor, and it is $>0$.

**What the descent looks like (13 restarts, randomized tiebreak).** BIMODAL: $\sim$4/13 restarts find a SPARSIFICATION CASCADE (enough rewires break the critical core, then pure removes avalanche, $m\,223\to164$, excess to **359-409**); $\sim$9/13 stall early in a dense local minimum (excess $\sim$680, $m\sim$225). Stall range [359, 846]. At the floor the stall is GENUINE: a thorough confirming scan finds that EVERY remaining violating edge is $\chi$-critical (its removal drops $\chi$ to 5) AND has no codegree-safe forced-same compensator. The deepest floor $m=164$ sits ABOVE the Kostochka-Yancey 6-critical floor ($2.8n-1.8=129.8$), so it is not an edge-count wall; the graph is locally codegree-rigid.

**Two escape levers tried and rejected (opt-in, default off).** (1) TRADE rewires (compensator allowed to create a SMALLER violation than $e_1$ removed, $K_4$-safe only) reach only a DENSER local minimum (excess $\sim$453-477, $m\sim$220): greedily taking a density-preserving trade starves the sparsification cascade that the strict codegree-safe path needs, so the more-permissive search does WORSE. (2) VERTEX-SPLIT GROWTH (add a fresh vertex, move half a high-load hub's neighbors to it to halve its codegrees, restore $\chi$ with a forced-same compensator) does not fire usefully: splitting a near-critical graph drops $\chi$ below 6 and no single forced-same compensator restores it. So neither denser trades nor larger $n$ breaks the stall from this seed.

**Significance.** L63 showed BOTTOM-UP that manufactured $K_4$-free 6-critical hosts are too DENSE ($m/n\approx4.2$) for the $K_{2,3}$ ceiling. L69 shows TOP-DOWN that the canonical small $\chi=6$ graph's codegree violations cannot be repaired away without losing $\chi=6$: the high-codegree substructures ARE the chromatic engine in this region. The three directions triangulate the same obstruction: an in-class $\chi\ge6$ object, if it exists, lives in a region unreachable by local moves from any natural seed (Mycielski tower, empty graph, or annealed graph).

**Honest scope.** No bound moved. HEURISTIC negative (local search, not exhaustive, not a non-existence proof): the object may exist at a structure no local move reaches; in particular a SPARSE 6-critical graph that fits the codegree ceiling is arithmetically allowed at $n=47$ ($m\ge130$ feasible). The floor is a local-search floor and varies (bimodal); 359 is the best of 13 restarts, not a canonical invariant. **Wrong-approach detectors**: N/A (in-class construction search on abstract graphs, no UDG embedding, no bound).

**Addendum (the SPARSIFICATION operator, decisive: violations live in the 6-critical core).** The L69 follow-up added the operator that should have broken the stall: remove a REDUNDANT (non-violating, $\chi$-preserving) edge to drop $m$ toward the Kostochka-Yancey 6-critical floor and free up the violating-edge sweep ([`try_sparsify`](combinatorial/e16_topdown_repair.py), `E16_SPARSIFY=1`). It works perfectly and the result is sharper than expected: from the $m=164$ floor it strips **all 33 redundant edges down to $m=131$ -- essentially AT the KY floor $2.8\cdot47-1.8=129.8$, i.e. a maximally sparse edge-critical 6-chromatic graph -- while the codegree excess stays PINNED at 359 (166 violating pairs, max codegree 10), reduced by ZERO.** Independently portfolio-verified (committed artifact `combinatorial/e16_repair_floor.json`, updated to this $m=131$ version): $K_4$-free, $\chi=6$ (5-UNSAT + 6-SAT), edge-critical, still NOT $K_{2,3}$-free. So the $K_{2,3}$ violations are not removable padding -- they are intrinsic to the 6-CRITICAL CORE, and the chromatic obstruction and the codegree violations are the SAME substructure here. Since every $\chi=6$ graph contains a 6-critical subgraph and a subgraph's $K_{2,3}$ persists in any supergraph, NO $\chi=6$ graph reachable from $M^3(C_5)$ by repair is in-class at $n=47$: the obstruction is the core itself, not the surrounding density. (Side note: this $m/n=2.79$ edge-critical $K_4$-free 6-critical graph is much SPARSER than L63's Mycielski-shrink cores at $m/n\approx4.2$, so those were not minimal; sparse 6-critical graphs do exist, they are just still codegree-violating at $n=47$.) Honest caveat: this concerns the $M^3(C_5)$-derived family at $n=47$; it does not prove EVERY 6-critical graph at $n=47$ has a $K_{2,3}$ (that needs enumeration / nauty, absent on this host). Next: the only remaining door is a NON-Mycielski sparse $\chi=6$ seed or genuinely larger $n$ (where the codegree ceiling $\sim0.7n^{1.5}$ outruns the KY floor $2.8n$, so $K_{2,3}$-free 6-critical cores must eventually exist) -- but sourcing a sparse 6-critical graph outside the Mycielski family needs the nauty capability this host lacks.

---

### L68. The solver ceiling is broken, but not where L66 predicted: watched-literal nogood propagation (the named lever) was implemented and STILL loses to pure CBJ, because the real bottleneck is WEAK conflict clauses, not slow propagation; the actual world-class win is to EXPORT hn_solver's native symmetry break as CNF and let a production CDCL engine (Cadical) do the learning, which decides **$M^4(C_5)$ $k=6$ UNSAT in 21.9 s and $P_{510}$ $k=4$ UNSAT in 1.66 s, both previously intractable**. Two products. (1) NEGATIVE-as-built: [`hn_solver_wl.py`](_shared/hn_solver_wl.py), a two-watched-literal version of the CBJ+nogood solver, integer literal ids ($L=\text{pos}\cdot k+c$) and flat list-of-lists indexing so PyPy JITs the propagation loop. Validated correct (0 verdict disagreements vs `hn_solver`, `hn_solver_cdcl`, and the portfolio over 300 graphs $\times$ 4 colors + witness validity). It beats the `hn_solver_cdcl` linear scan ~2.3x under PyPy (M^3 nogood path $0.56\to0.24$ s) but STILL loses to pure CBJ (0.16 s): 87,621 learned-path nodes at 359k n/s $=0.24$ s vs 516,549 bare nodes at 3.28M n/s $=0.16$ s. L66 diagnosed the bottleneck as the O(nogoods) scan; the truer diagnosis is that a Prosser conflict set is a large, non-asserting clause, so even at O(1) it cannot cut enough nodes to repay its per-node cost. 1UIP-quality learning is needed, and Python caps per-node speed regardless. (2) THE WIN: [`build_color_cnf_symbreak`](_shared/portfolio_sat.py) emits the clique-fixing + first-appearance-precedence clauses hn_solver searches natively as a CNF preamble, so Cadical inherits the color-permutation symmetry break the naive one-hot encoding throws away, on top of its own 1UIP learning / watched literals / VSIDS / restarts. Validated EQUISATISFIABLE with the naive encoding (0 disagreements, 0 bad witnesses, 1000+ instances). Measured (Cadical195, single isolated run): $M^3(C_5)$ $k=5$ UNSAT naive 7.0 s $\to$ **0.05 s (~146x)**; $M^4(C_5)$ $k=6$ UNSAT naive WALL $\to$ **21.9 s**; $P_{510}$ $k=4$ UNSAT naive $\sim$110 s $\to$ **1.66 s**; $P_{510}$ $k=5$ SAT instant.

**Architecture**: 1 (combinatorial / solver tooling). Scripts: [`hn_solver_wl.py`](_shared/hn_solver_wl.py) (watched-literal solver + self-test), [`portfolio_sat.py`](_shared/portfolio_sat.py) (`build_color_cnf_symbreak` + `colorable_portfolio(symbreak=True)`), [`symbreak_bench.py`](_shared/symbreak_bench.py) (equisat validation + wall benchmark). White paper room-for-improvement #1 (watched literals) and #7 (symmetry-broken export) both updated to DONE.

**Significance for the program.** (a) The genuine computational wall $M^4(C_5)$ $k=6$ UNSAT falls (an independent, certificate-capable proof that $\chi(M^4(C_5))\ge7$, never finished by pure hn_solver or the naive portfolio); and $P_{510}$ $k=4$ UNSAT, slow naively ($\sim$110 s), drops to a 1.66 s independent re-certification that $P_{510}$ is genuinely $\chi=5$. (Correction, C3-paper adversary pass: $P_{510}$ $k=4$ is NOT a wall and was NOT the L29/L30 "SAT-intractable" instance -- that was the 510$\times$517 $\chi\ge6$ coupling, L34; the earlier "wall" label conflated the two.) (b) This is the right default for the L64 in-class E14 decisions: handing every portfolio member the symmetry break shrinks the Cadical-vs-MapleChrono heuristic swing that was L64's central worry. (c) The methodological law generalizes L66: implement the named lever and MEASURE before believing the diagnosis; the watched-literal lever was correctly implemented and correctly measured to be insufficient, which is what redirected effort to the export route.

**Honest scope.** No bound on $\chi(\mathbb{R}^2)$ moved; this is solver tooling. The symmetry break is a standard sound construction (no novelty claim on the encoding itself); the contribution is wiring the program's native break into the portfolio and measuring that it crosses the walls. The naive baseline for $M^4$ is a genuine WALL (does not finish); $P_{510}$ $k=4$ naive was at first recorded as WALL because it exceeds a ~120 s conflict budget, but the C3-paper adversary pass later MEASURED it at $\sim$110 s (a slow finish, right at the budget boundary, NOT an intractable wall) -- so only $M^4$ is a true wall here. **Wrong-approach detectors**: N/A (colorability decision tooling on existing graphs, no construction, no bound). Next: route the E14/E15 in-class decisions through `symbreak=True`; consider emitting DRAT proofs from the symmetry-broken UNSAT runs for certificate-grade records; optionally a maximum-clique seed for an even stronger break.

**Addendum (all three follow-ups landed).** (1) ROUTED: the E14 decisive solves now use the symmetry-broken encoding (`e14_udg_class_host.solve5`; `e14b_overshoot.budgeted_solve` default `symbreak=True`, inherited by `e14c`), while `sample_pool` deliberately stays naive (it needs DIVERSE colorings that symmetry breaking canonicalizes away, and its random color assumptions fight the canonical frame); `e15_min_inclass_chi6.confirm` cross-checks 5-UNSAT/6-SAT with `symbreak=True`. (2) DRAT CERTIFICATES: `solve_color(..., proof_path=...)` (pysat `with_proof`) emits a DRAT proof on UNSAT; the E14b/E14c success paths and E15 `confirm` write it automatically for any found in-class $\chi\ge6$ witness. The break shrinks proofs ~66x ($M^3$ k=5: 13,263 lines vs 879,467 naive), and $M^4(C_5)$ k=6 UNSAT produced a complete **1,944,586-line DRAT proof in 26 s** (a concrete machine-checkable record that $\chi(M^4(C_5))\ge7$; proofs land in gitignored `_cache`). Honest scope: the proof certifies the symmetry-broken CNF; the symmetry clauses are sound by construction, so it entails non-colorability, but it is not a DRAT proof of the naive CNF (intractable, the point); preprocessing-closed instances leave an empty trace (no file). (3) MAX-CLIQUE SEED: `build_color_cnf_symbreak` now pins an EXACT maximum clique on the high-degree core (default; greedy fallback), pinning a full triangle on triangle-rich in-class UDGs (clique [1,6,9] on the e15b n=29 graph) where greedy might find a smaller one; a no-op on the triangle-free tower. Re-validated equisatisfiable (300 graphs x 4 k, 0 disagreements / 0 bad witnesses). White-paper room-for-improvement #4 also marked DONE.

**Addendum 2 (what the tool can now solve, and the regime boundary it does NOT cross).** A capability battery with the symmetry-broken encoding settled both halves. CAN solve (the entire known $\chi(\mathbb{R}^2)\ge5$ computational program is now self-certifiable on one machine, each with an optional DRAT proof): **de Grey 1585 k=4 UNSAT in 18.1 min** (1088 s, Cadical195, symbreak, vs 92.2 min naive; an independent, from-the-graph confirmation that $\chi(\mathbb{R}^2)\ge5$; persisted+reproducible via [`e1u_degrey_symbreak_k4.py`](combinatorial/e1u_degrey_symbreak_k4.py), cache `_cache/e1u_degrey_1585_symbreak_k4.json` -- this measured value replaces the earlier prose-only "19.5 min" estimate, flagged as unbacked by the C3-paper adversary pass); ALL NINE $\chi$-5 lineage graphs k=4 UNSAT in **1.5-9 s each** (510, 517, 529, 553, 610, 633, 803, 826, 874 -> each certified $\chi\ge5$); $M^4(C_5)$ k=6 UNSAT 22 s (1.94M-line DRAT proof). CANNOT solve (the boundary, important and honest): the E14 OVERSHOOT instances -- $P_{510}$ plus $\sim$1000 codegree/$K_4$-safe edges at k=5, sitting on the 5-SAT/5-UNSAT PHASE BOUNDARY -- are a money pit for BOTH encodings (naive Cadical did not finish in $\sim$100 min; symbreak hit BUDGET at 500k/2M/10M conflicts = 29 s/157 s/1387 s, undecided). So symmetry breaking crushes STRUCTURED UNSAT (sparse, clearly-UNSAT lineage/tower/de Grey) but does NOT tame PHASE-TRANSITION hardness; they are different difficulties and conflating them was a (corrected) over-optimism. This means the E14/E15 search for a NEW small in-class $\chi\ge6$ object is NOT unblocked by symbreak; the obstruction is intrinsic (consistent with L67's "liquid" class never reaching $\chi=6$ in 19,330 restarts, and L64's heavy-tailed near-boundary runtimes), not a solver artifact. SIDE BUG FIXED: E14c hung not on mathematics but because its solver MapleChrono does NOT honor pysat `conf_budget` (ran 45 min on a 2M-conflict probe); E14b/E14c default solver switched to Cadical195, which respects the budget and returns BUDGET cleanly so the overshoot can jump past the band instead of hanging (this was the real "money pit" mechanism of L64, distinct from the machine-sleep kill). Net for the program: a complete certification capability for everything KNOWN; the search for the unknown object still needs a new idea, not a faster decision procedure.

---

### L67. E15b (overnight annealing, the E15 redirect): simulated annealing over the UDG-necessary class reaches in-class $\chi=5$ down to $n=29$ but NEVER $\chi=6$ across 19,330 restarts, sharpening the greedy-$\chi=4$ cap of E15 by one color and giving a strong (heuristic) negative on the paper's target object at $n\le40$. Local search (add AND remove codegree-safe + $K_4$-safe edges, escaping E15's greedy add-only trap) on $n=26$-$40$, 8 h under PyPy, 1289 rounds, objective = minimize the capped 5-coloring PATTERN count (hn_solver's counter; 0 $=$ $\chi\ge6$) with a density tiebreak. Result: max $\chi$ reached $=5$; density plateaus at a remarkably uniform $\sim0.84$ of the codegree ceiling $n(1+\sqrt{8n-7})/4$ at every $n$; the smallest in-class $\chi=5$ graph found is **$n=29$, $m=98$**, INDEPENDENTLY verified (K4-free, $K_{2,3}$-free, 4-UNSAT, 5-SAT) by the CPython portfolio, committed as `combinatorial/e15b_chi5_n29.json`. So the in-class chromatic ladder by construction method is: greedy-from-empty $\to\chi=4$ (L65), annealing $\to\chi=5$ (here), and $\chi=6$ resists.

**Architecture**: 1 (combinatorial). Script [`e15b_anneal_inclass.py`](combinatorial/e15b_anneal_inclass.py) (self-contained, PyPy-runnable: imports only hn_solver, no pysat); cache `_cache/e15b_anneal/` (`SUMMARY.json`, `BEST.json`, gitignored). PyPy made the 19,330-restart budget feasible in 8 h (L66).

**Honest scope.** No bound moved, and this is a HEURISTIC negative: annealing's failure to reach $\chi=6$ is NOT a non-existence proof. The in-class $\chi\ge6$ object may live at $n>40$, or at a density/structure the local search does not reach (the $\sim0.84$-of-ceiling plateau suggests annealing cannot push the last $\sim16\%$ of density, where a $\chi=6$ witness would most plausibly sit). It DOES show the object is not easily constructed at small $n$, consistent with the codegree squeeze (L63) and the lineage liquidity (L64). The verified $n=29$ in-class $\chi=5$ graph is itself a small artifact (the only previously exhibited in-class $\chi=5$ graphs are the 510-vertex Polymath lineage). **Wrong-approach detectors**: N/A (in-class construction search, no bound). Next: E15-v2 violation-minimization from a NON-critical 6-chromatic seed (a different escape from greedy, still untried), and/or extend the annealing to $n>40$ or push the density plateau with smarter moves.

---

### L66. Conflict-directed backjumping + nogood learning added to hn_solver, with a careful parameter sweep that overturned a premature conclusion: CBJ (fixed order + backjumping) is the REAL win (2.6-5.3x faster than the MRV chronological solver on hard structured instances), nogood learning reduces NODES (6.3x) but its wall-time benefit is gated on watched-literal indexing Python lacks, and $M^4(C_5)$ still needs a compiled core. New module [`hn_solver_cdcl.py`](_shared/hn_solver_cdcl.py): Prosser CBJ + nogood learning (each conflict set recorded as a forbidden (vertex,color) combination, indexed and reused to prune). Validated CORRECT: zero verdict disagreements vs the chronological hn_solver AND the MapleChrono portfolio over 200 random graphs x 3 colors, plus valid witnesses. Soundness fact making conflict analysis clean under symmetry breaking: a vertex can always take color $(\max\text{ used})+1$ unless that hits $k$, so a wipeout occurs ONLY when its neighbors already occupy all $k$ colors (a pure graph conflict, conflict set = one neighbor per color).

**The measured result (and a corrected mistake).** A first reading defaulted to max_learn=20 and concluded "wall-time neutral." A parameter sweep on $M^3(C_5)$ $k=5$ UNSAT corrected this: chronological MRV solver 447,720 nodes / 13.3 s; pure CBJ (max_learn=0) 516,549 nodes but **1.2 s (11.4x faster)** because the fixed-order per-node cost is far below MRV's per-node minimum-remaining-values scan; max_learn=8 reduces to 81,850 nodes (6.3x fewer) but 2.5 s (the nogood checks cost more per node); max_learn=20 bottoms out at 66,651 nodes but 17.7 s (per-node cost balloons, NET SLOWER). So the headline win is BACKJUMPING + cheap fixed-order, not the nogoods. Multi-instance (max_learn=8 vs chronological): $M^2$ 2.6x, $M^3$ 5.3x, rand G(40,.3) 2.5x, but rand G(60,.25) 0.2x (SLOWER: there MRV's dynamic ordering nails it in few nodes and CBJ's fixed order plus nogood overhead lose). Net: CBJ wins where the search is genuinely hard (large tree), loses where smart ordering resolves quickly. $M^4(C_5)$ $k=6$ UNSAT stays out of reach in pure Python: pure CBJ needs $>$60M nodes (325k n/s, did not finish in 185 s); max_learn=8 did not finish 25M nodes in 300 s (learning's node reduction eaten by per-node cost).

**Architecture**: 1 (combinatorial / solver tooling). Module + self-test [`hn_solver_cdcl.py`](_shared/hn_solver_cdcl.py) (default max_learn now 8, with a measured tuning note); white-paper room-for-improvement #1 updated. **Honest scope.** No bound moved; a tooling result. The lesson: SWEEP the parameter before concluding (the max_learn=20 default hid an 11x CBJ win). Learning's node reduction is real but its wall-time payoff needs watched-literal nogood propagation (O(1) checks instead of a linear scan) and/or a Rust/PyO3 core; $M^4$ needs both. CBJ alone is now the recommended fast config; nogoods are for node-count studies.

**Addendum (PyPy, the free constant factor).** Installed PyPy 3.11 (winget `PyPy.PyPy.3.11`) and benchmarked the pure-Python core ([`pypy_bench.py`](_shared/pypy_bench.py); no pysat/numpy, so the JIT applies fully). On $M^3(C_5)$ $k=5$ UNSAT, pure CBJ, the SAME 516,549-node search (node counts made interpreter-reproducible by choosing the smallest-position conflict blamer via a sorted tuple): CPython 0.87 s vs **PyPy 0.17 s = 5.0x faster, zero code changes** (3.4x on the slower nogood path). STACKED, on $M^3$: old MRV chronological 13.3 s $\to$ CBJ 0.87 s (15x, backjumping) $\to$ +PyPy 0.17 s (**~78x total**, all in Python), and $\sim$195x faster than MapleChrono on this structured instance (the symmetry breaking the naive SAT encoding lacks). BUT $M^4(C_5)$ $k=6$ UNSAT STILL did not finish under PyPy: pure CBJ hit a 400M-node budget at 1.59M n/s in 252 s. The NODE COUNT is the wall, not per-node speed, so PyPy's 5x is not enough alone. Remaining lever priority confirmed: (1) watched-literal nogood propagation (make learning's measured 6.3x node reduction pay off), still Python; (2) only then a Rust/PyO3 core. Deployment caveat: PyPy accelerates only the pure-Python solver; pysat (portfolio, forcing census) is a C extension that runs poorly under PyPy, so keep those on CPython.

---

### L65. E15 + a from-scratch solver + the lemma's provenance: three results from the "build our own tools" turn. (1) E15 (NEGATIVE, quantitative): GREEDY in-class growth caps at $\chi=4$. Building from the empty graph by adding only $K_4$-safe AND codegree-safe ($K_{2,3}$-free) edges, across orders $n=17$ to $64$ with 16 restarts each, NEVER reaches even $\chi=5$, let alone 6: every restart gets STUCK at $\chi=4$ at density $m/\text{ceiling} \in [0.42, 0.68]$ (decreasing in $n$), well below the codegree ceiling. So $\chi\ge5$ is UNREACHABLE in the UDG-necessary class by local edge-addition from scratch. This is a greedy limitation, NOT a non-existence proof (in-class $\chi=5$ graphs DO exist: $P_{510}$ is one, L64), but it sharply quantifies why the only known in-class $\chi=5$ graphs are the large GEOMETRICALLY-constructed de Grey/Polymath lineage, not anything a local search stumbles into. The "deep liquidity" (L57/L62) now shown to extend to construction: you cannot greedily assemble a high-chromatic graph under the codegree cap. (2) A FROM-SCRATCH solver (`hn_solver.py`): a k-colorability decider built on the program's own structure, NOT a SAT wrapper. Color-class symmetry breaking (a vertex may introduce color $c$ only if $c\le 1+\max$ used, collapsing the $k!$ permutation symmetry a one-hot SAT encoding carries) + greedy clique seeding ($\omega>k\Rightarrow$ instant UNSAT) + MRV branching + a coloring-PATTERN counter (proper colorings up to color permutation, which SAT solvers do not give and which directly measures liquidity). Cross-validated: 0 disagreements vs the portfolio over 60 random graphs $\times$ 2 $k$-values, valid witnesses, pattern counts matching brute force ($C_5$ has exactly 5 three-coloring patterns). It DROVE the entire E15 sweep in-process (no subprocess spawn). (3) PROVENANCE (surveyor lit-check): the Essential-Pair Lemma is NOT new. Its corollary is Theorem 3.17 ("there are no implicit relations in critical graphs") of Martin, "Graph-Chromatic Implicit Relations" (arXiv:0901.1255, 2009), where forced-different/forced-same pairs are "implicit-edges"/"implicit-identities"; also implicit in Dirac-Gallai critical-graph theory (Jensen-Toft 1995). The paper was corrected to attribute it and make NO priority claim; the contribution narrows honestly to the exhaustive census + the "forcing-sterile by construction" APPLICATION to the lineage + the codegree synthesis.

**Architecture**: 1 (combinatorial). Scripts [`e15_min_inclass_chi6.py`](combinatorial/e15_min_inclass_chi6.py), [`hn_solver.py`](_shared/hn_solver.py); provenance note `sources/notes/essential_pair_lemma_provenance.md`; results `_cache/e15_min_inclass_chi6/SUMMARY.json`. The paper ([`paper/main.tex`](../paper/main.tex)) now compiles clean with the Martin/Jensen-Toft citations and the corrected nine-graph census total (1,955,948, not the twelve-graph 2,309,264).

**Addendum (hn_solver benchmarked + white paper).** A 24-instance benchmark ([`hn_solver_bench.py`](_shared/hn_solver_bench.py), results `_shared/hn_solver_bench_results.md`) vs MapleChrono: ZERO correctness disagreements (84 cross-validations incl. the self-test), and hn_solver is FASTER on 21/24, by 100-1000x on structured/random instances (rand G(40,0.3) $k=5$ UNSAT 4.4ms vs 5.43s; $M^2(C_5)$ 130x; $P_{510}$ $k=5$ SAT 55x). The win is over the NAIVE one-hot SAT encoding (which carries the full $k!$ color symmetry hn_solver breaks natively); a symmetry-broken CDCL encoding would close most of the gap. CEILING (honest): no clause learning, so the Mycielski node count explodes (882 at $M^2$, 447,720 at $M^3$, intractable at $M^4$ $k=6$ and $P_{510}$ $k=4$). White paper with the full design + room-for-improvement (clause learning is the #1 next step): [`docs/03_research/hn_solver_whitepaper.md`](../docs/03_research/hn_solver_whitepaper.md). Best deployment: a structural FRONT END to the portfolio (resolve the structured-easy majority instantly, hand the hard residue to CDCL).

**Honest scope.** No bound moved. E15's $\chi=4$ cap is a greedy/local-search result, decisive about local construction but silent on existence of small in-class $\chi\ge5$ (still open; would need a smarter search, e.g. violation-minimization from a non-critical 6-chromatic seed, or a CP search over graph space). hn_solver is a research instrument (correct, structure-exposing), not a speed competitor to MapleChrono at lineage scale. The lemma provenance is settled (known). **Wrong-approach detectors**: N/A (in-class construction search + solver, no bound). Next: E15-v2 violation-minimization to actually FIND a small in-class $\chi\ge6$ object (the paper's target), and/or feed hn_solver's pattern-counter into a liquidity measurement of the lineage.

---

### L64. E14 (L63 redirect executed, VERDICT OPEN): generating INSIDE the UDG-necessary class ($K_4$-free AND $K_{2,3}$-free) from a real seed does not cheaply force $\chi\ge6$ -- the class is LIQUID -- and the binding constraint on deciding it turned out to be SAT-SOLVER STRATEGY, not mathematics. Two concrete products, one negative-leaning observation, one sharp methodological law. (1) SEED LOGIC CONFIRMED: $P_{510}$ (a real Polymath16 UDG) is automatically $K_4$-free AND $K_{2,3}$-free, verified at load (codegree $\le 2$ everywhere). So the realizable lineage lives natively in the class L63 identified; growth never has to leave it. (2) THE CLASS IS LIQUID: adding 524 model-guided plus blind codegree-safe + $K_4$-safe edges to $P_{510}$ (reaching 674 added, $m=3178$ on 510 vtx, still vastly below the $\sim$8200 codegree ceiling) keeps the graph 5-COLORABLE at every step, with NO stuck signal (safe-edge headroom never ran out). Forcing $\chi\ge6$ inside the class from this seed is hard, consistent with the L45/L56/L57 "deep liquidity" picture: the realizable neighborhood resists long-range forcing even when you actively push edges into it. (3) THE PORTFOLIO INVERSION (the session's most concrete finding): on the IDENTICAL 470-edge instance, **Cadical ran 12+ hours without deciding while MapleChrono returned SAT in 155 seconds** -- a $\ge$280x heuristic-dependent swing, exactly the heavy-tailed runtime behavior expected AT a SAT phase boundary. This retroactively explains the h6 $510\times517$ instance abandoned as "SAT-intractable" after 66 Cadical-minutes (L29/L30): it was very likely tractable for a different solver and was never re-tried. LAW: near-threshold $k$-colorability instances in this program MUST be run as a solver PORTFOLIO (Cadical for DRAT certification, MapleChrono/Glucose for discovery), never a single solver.

**Architecture**: 1 (combinatorial). Scripts [`e14_udg_class_host.py`](combinatorial/e14_udg_class_host.py) (grower, in-class invariants as hard constraints), [`e14b_overshoot.py`](combinatorial/e14b_overshoot.py) (model-guided overshoot), [`e14c_overshoot_jump.py`](combinatorial/e14c_overshoot_jump.py) (aggressive-jump overshoot). State/logs under `_cache/e14_udg_class/` (`STATE.json` 470, `STATE_B.json` 524, `STATE_C.json` 674; gitignored). DIMACS of the 470-edge instance persisted for solver handoff.

**The overshoot principle (method, validated in shape).** Walking up to the UNSAT boundary one edge at a time pays the threshold tax EVERY step: solve time on the in-class instances exploded $130\,\text{s}\to8631\,\text{s}$ over $\sim$54 added edges (524 edges $=$ 2 CPU-HOURS for a single decisive solve). The hard region is a BAND around the threshold; solves on either side (comfortably colorable, comfortably overconstrained) are fast. So the right algorithm JUMPS past the band with free, no-solve blind safe-edge additions, then solves once on the cheap overconstrained side; a later shrink recovers minimality with UNSAT re-checks that are cheap there. Empirically the band is WIDE: a $+70$-edge jump from 524 did not clear it (still solving after 5 min), so E14c uses $+150$ jumps with cheap (1.5M-conflict) probes that return fast and trigger the next jump. The verdict (UNSAT $=$ first 6-chromatic graph in the UDG-necessary class; or STUCK $=$ the class caps below 6 from this seed, the UDG analog of $\chi(\mathbb{Q}^2)=2$) is STILL OPEN at write time.

**Honest scope.** NO bound moved; the central E14 question is UNDECIDED (neither UNSAT nor STUCK reached). The liquidity observation is suggestive, not a theorem: 674 safe edges added without forcing $\chi\ge6$ shows the class is hard to push from $P_{510}$, NOT that it caps at 5. The portfolio inversion is a real, reproducible methodological result (two solvers, one instance, persisted DIMACS) and is the most citable thing here. **Wrong-approach detectors**: N/A (in-class decision search, no construction, no bound). Operational footnote: the first overnight E14c run was killed not by mathematics but by the machine sleeping (froze the solver mid-solve) then a Windows auto-update reboot at 10:28 the next morning; compute-hardening (disable sleep, defer reboot) is now a prerequisite for unattended runs. See [`docs/03_research/E14_UDG_class_and_portfolio_findings.md`](../docs/03_research/E14_UDG_class_and_portfolio_findings.md) for the full write-up.

---

### L63. E13/E13b (L61 route 1 executed): the nauty-free host FACTORY works -- Mycielski-tower seeds plus SAT-guided shrink yield triangle-rich K4-free 6-critical hosts down to **18 vertices** with ABUNDANT alternators -- but a NEW quantitative wall excludes every host found from UDG-realizability: the **codegree bound**. In a planar UDG two unit circles meet in at most 2 points, so every vertex pair has codegree $\le 2$ (every UDG is $K_{2,3}$-free), forcing $m \le n(1+\sqrt{8n-7})/4$. No manufactured host gets near it: the n=18 host is EDGE-critical at $m=76 > 57 = $ ceiling(18); all ~35 independent edge-critical cores (n=18-24, both seed lineages, codegree-aware and unaware searches alike) land at density $m/n \approx 4.2$ with 126-148 codegree violations; and $M^3(C_5)$ itself has **434 violations** (max codegree 11), retroactively explaining the L52/L61 realization hopelessness on independent grounds. Corollaries: (rigorous, one line) any 6-chromatic UDG has a 6-critical core on $n \ge 13$ vertices (the Kostochka-Yancey floor $2.8n-1.8$ exceeds the $K_{2,3}$ ceiling for $n \le 12$); (empirical, conditional) if K4-free 6-critical graphs really need $m/n \approx 4.2$ at this scale, UDG-plausible hosts need $n \ge \sim 32$, so the phase-route gadget will NOT be tiny and the 47-48-vtx scale of past attempts was never the mistake.

**Architecture**: 1 (combinatorial). Scripts [`e13_small_hosts.py`](combinatorial/e13_small_hosts.py) (257 s) and [`e13b_edge_minimal.py`](combinatorial/e13b_edge_minimal.py) (174 s); hosts with full edge lists committed in `e13_hosts.json` / `e13b_hosts.json`; logs and JSON under `_cache/e13_small_hosts/`, `_cache/e13b_edge_minimal/`.

**The factory (positive half).** Seeds $M(M(W_5))$ (27 vtx) and $M(M(C_5)+K_1)$ (25 vtx) are K4-free, 6-chromatic, VERTEX-CRITICAL, with 80 triangles covering 24/27 resp. 24/25 vertices. Mycielski preserves K4-freeness (a K4-free graph has triangle-free neighborhoods, so no shadow clique can close) while the wheel hub / apex inject triangles: exactly the $\omega=3$ substrate L61 demanded, with no enumerator needed. Shrink move: delete a vertex (criticality makes $G-v$ 5-colorable), push back to 5-UNSAT by adding K4-safe blocking edges between same-colored non-adjacent pairs of successive SAT models, extract the vertex-critical core, recurse. 348 restarts in 240 s; stuck-size histogram 18:1, 19:17, 20:90, 21:165, 22:61, 23:5, 24:9. Best host: n=18, m=79 (76 after edge-minimization), 84 triangles covering ALL 18 vertices, degrees [7,12].

**Alternators transfer to the new substrate.** Census on the cores (diff-diff is the host itself, auto-5-UNSAT; merge-both filtered by 1 SAT call; non-degeneracy SAT-checked exactly, since vertex-critical does not imply edge-critical): 626/2449 disjoint edge pairs at n=18 pre-minimization (~26%, vs 2.8% on $M^3(C_5)$), 438-706 on the edge-critical cores. The n=20 core has $\|\mathrm{Aut}\|=10$ with 3 of 5 sampled alternators carrying a pair-swap involution: the L61 congruence advantage survives in the triangle-rich family. Triangle-rich critical hosts are MORE alternator-abundant, not less.

**The codegree wall (negative half, the finding).** Edge-minimization with violation-first deletion order cannot push density into the $K_{2,3}$ window: the squeeze is $2.8n - 1.8 \le m \le n(1+\sqrt{8n-7})/4$, open arithmetically from n=13, but every reachable host sits at $m/n \approx 4.2$, i.e. avg degree $\approx 8.4$, which needs $n \ge 32$ before the ceiling clears it. The density is structural (edge-critical, no slack), not an artifact of the blocking heuristic: codegree-aware blocking (hard K4 filter, soft codegree filter) lands in the same place. $L^\infty$ contrast: sup-norm unit "circles" are squares and can share a SEGMENT, so codegree is unbounded there; the bound is genuinely Euclidean two-circle rigidity (detector-ALIGNED). It claims nothing on $\mathbb{Q}^2$ / $\mathbb{R}^1$ (it is an obstruction on hosts, not a lower-bound method).

**What this redirects (E14, named).** Stop manufacturing hosts ABOVE the UDG class and cutting down; generate INSIDE the UDG-necessary class with $K_4$-freeness AND $K_{2,3}$-freeness as HARD invariants. Seed from a real UDG ($P_{510}$: chi-5, automatically both-free), add ONLY codegree-safe + K4-safe blocking edges until 5-UNSAT, then vertex+edge minimize (deletions preserve both freenesses). Output: the smallest reachable 6-chromatic graph passing every local UDG obstruction, its alternator census, and its density (does the class force $n \ge \sim 32$?). Secondary: prototype the necklace transfer-digraph imprimitivity machinery on the n=20 involution host (abstract, cheap, de-risks the phase pipeline before any W3 attempt).

**Honest scope.** No bound moved. The codegree ceiling is necessary, not sufficient (the true max unit-distance edge count $u(n)$ sits far below the counting ceiling at small n); "density $\approx 4.2$" is an observation on ~35 cores from ONE construction family, not a theorem about all K4-free 6-critical graphs. Literature anchor for a future SURVEYOR pass: the minimum order of a K4-free 6-chromatic graph is the vertex Folkman number $F_v(2,2,2,2,2;4)$; our n=18 is an upper-bound demonstration, not a record claim. The open object is sharpened: a 6-chromatic, alternator-bearing graph in the UDG-necessary class ($K_4$-free $+$ $K_{2,3}$-free) at the smallest reachable n.

*(Addendum, same session: literature anchor resolved. $F_v(2,2,2,2,2;4) = 16$ exactly, and the only 16-vtx witnesses are the two Ramsey (4,4)-graphs on 16 vertices (no $K_4$, no independent 4-set, so $\chi \ge \lceil 16/3 \rceil = 6$; Paley-type, $m \approx 60$, avg degree $\approx 7.5$, adjacent codegrees 3-4). Both violate the codegree ceiling ($60 > 48 = $ ceiling(16)), so the UDG-necessary class has NO member at the Folkman floor: rigorously $n \ge 17$ inside the class, and since the ceiling only reaches avg degree 7.5 at $n \ge \sim 26$, even Folkman-floor density excludes the class below $n \approx 26$. Our n=18 factory output is 2 above the true floor (near-optimal already); L51's abstract floor $[9,48]$ is settled at 16 by literature, no nauty needed. Sources: Coles-Radziszowski, On minimal Folkman graphs; Xu-Liang-Radziszowski, Chromatic Vertex Folkman Numbers.)*

---

### L62. E12 (L61 route 2 executed): the BORN-REALIZED equality-alternator is ABSENT from the realizable lineage, extending L57/L59 sterility one logical level up (from single pairs to PAIRS-OF-PAIRS / two-edge forcing). Across all 9 chi-5 graphs (top-60-degree core, 500 diverse 5-colorings each): ZERO diff-diff candidates, hence zero alternators. So the lineage hosts no joint-pattern forcing either: there is no pair of disjoint non-adjacent pairs $(e_1,e_2)$ with $G+e_1+e_2$ 5-UNSAT. Like the clamp, the alternator must come from a NEW graph; the phase route gets no free born-realized object from known UDGs. The per-graph mono-rate is $\sim[0.10,0.37]$, centered $\sim0.2$, IDENTICAL across all 9 (L57's "deep liquidity" confirmed independently and quantitatively).

**Architecture**: 1 (combinatorial). Script [`e12_born_alternator.py`](combinatorial/e12_born_alternator.py) (~60 s for all 9). Results `_cache/e12_born_alternator/SUMMARY.json`.

**Method (sampling filter, decisive with no false negatives for true alternators).** Brute pairs-of-pairs on 510-874 vtx is too slow, so: sample 500 diverse proper 5-colorings (random per-vertex color assumptions); a pair-of-pairs $(e_1,e_2)$ is a diff-diff CANDIDATE only if NO sampled coloring has both pairs bichromatic (i.e. $\text{mono}(e_1)\lor\text{mono}(e_2)$ holds on every sample). A TRUE alternator has $P(\text{both bichromatic})=0$ exactly, so it ALWAYS survives the filter (no false negatives); survivors would be SAT-confirmed. Zero survivors $=$ zero real alternators among core pairs. The high-degree core is where forcing concentrates (L42/L45), so it is the right place to look first.

**Why it lands negative (consistent with L59).** A born-realized alternator needs $G+e_1+e_2$ 5-UNSAT, i.e. $G$ must contain (K4-free 6-critical minus two independent edges) as a subgraph with $e_1,e_2$ the missing edges. The lineage graphs are vertex-critical chi-5 (L59): barely 5-chromatic, and they evidently contain no such near-6-critical substructure reachable by adding two core edges. This is the two-edge analog of L57 (one added edge never 5-UNSAT) and dovetails with the Essential-Pair Lemma.

**Honest scope / caveats.** (a) Restricted to pairs with BOTH endpoints' pairs in the top-60 core; an alternator using lower-degree vertices is not excluded (but forcing concentrates in hubs). (b) Sampling tests a necessary condition; it cannot miss a true alternator but the core restriction is a genuine scope limit. Net: a clean negative consistent with the entire L45/L57/L59 sterility picture; no bound moved. **Wrong-approach detectors**: N/A (decision sweep over realizable graphs). Both L61 routes now mapped: route 2 (born-realized in lineage) NEGATIVE here; route 1 (rebuild on a small $\omega=3$ 6-critical host) still open and now the sole live phase-route door, gated on a SOURCE of small triangle-rich K4-free 6-critical graphs (no nauty on this host).

---

### L61. E1b (attacking the alternator's W3): the equality-alternator shares the CLAMP's exact host-size bottleneck, plus one genuine structural advantage and one decisive substrate warning. Minimizing all 693 M^3(C5) alternators is VACUOUS: every one stays at 47 vertices, 234 edges, because the construction sits on a 6-CRITICAL host and deleting any vertex $v$ makes $K-v$ 5-colorable, which breaks the alternator's diff-diff condition ($G+e_1+e_2 = K-v$ must be 5-UNSAT). So the minimum alternator order EQUALS the minimum K4-free 6-critical order (L51's floor $[9,48]$, pinnable only with nauty, absent on this host), exactly the clamp's bottleneck. ADVANTAGE: $\sim$1/3 of the alternators (4 of the 12 smallest-distinct checked) carry a pair-swapping INVOLUTION (an automorphism $\sigma$ with $\sigma\{a_1,a_2\}=\{b_1,b_2\}$), a built-in congruence $\tau$ the clamp construction does not provide. WARNING: $M^3(C_5)$ is TRIANGLE-FREE ($\omega=2$), the worst possible UDG-realizability substrate (rigid unit triangles are what make UDGs realizable), so realizing the 47-vtx Mycielski alternator is as hopeless as the failed 48-vtx clamp (L52); the right substrate is a small $\omega=3$ (triangle-RICH, K4-free) 6-critical host.

**Architecture**: 1 (combinatorial). Script [`e11_alternator_w3.py`](combinatorial/e11_alternator_w3.py) (591 s; finds 693 alternators, minimizes each preserving A/B/C, computes pair-swap automorphisms via networkx VF2). Result `_cache/e11_alternator_w3/result.json`; full distinct set in `distinct_minimized.json`.

**Why minimization is provably vacuous on any 6-critical host.** The alternator gadget is $G = K - e_1 - e_2$ with $K$ K4-free 6-critical. Deleting a non-terminal vertex $v$ gives $(K-v)-e_1-e_2$; condition A (diff-diff forbidden) requires $(K-v)-e_1-e_2 + e_1 + e_2 = K-v$ to be 5-UNSAT, but $K-v$ is 5-colorable by criticality. So A fails for EVERY vertex deletion and nothing shrinks. (This is the alternator analog of L59: the same criticality that makes the lineage forcing-sterile makes the critical-host alternator irreducible.) Consequence: any non-degenerate alternator needs BOTH $e_1,e_2$ inside a single 6-critical subgraph (else $G+e_1$ or $G+e_2$ alone is already 6-chromatic $=$ a forced-same pair, which L57 forbids in realizable graphs), so the alternator's minimum order $=$ the minimum K4-free 6-critical order.

**What this redirects.** Two concrete next moves, both sidestepping the dead 47-vtx triangle-free target. (1) REBUILD on a small $\omega=3$ 6-critical host: triangle-rich, K4-free, 6-chromatic graphs are both smaller than 47 and far more realizable (triangles $=$ rigid unit rhombi/spindles); the obstacle is a SOURCE of such graphs without nauty (Hajos/Ore construction from small chi-5 pieces, or the repo's h5/h6 lineage). (2) BORN-REALIZED route (no W3 step at all): sweep the realizable chi-5 lineage for a JOINT-PATTERN alternator $=$ two disjoint non-adjacent pairs $(e_1,e_2)$ with $G+e_1+e_2$ 5-UNSAT and $G$ merged-on-both 5-UNSAT. This is the pair-OF-pairs property L57 never tested (it tested single pairs); equivalent to asking whether a realizable chi-5 UDG contains (K4-free 6-critical minus two independent edges) as a subgraph. Tractable as a sample: for each $e_1$ in the high-degree core, $G+e_1$ is still chi-5, so run the L56 forced-same sweep on $G+e_1$; any forced-same $e_2$ there gives a born-realized alternator.

**Wrong-approach detectors**: N/A (combinatorial decision + automorphism computation, no construction, no bound). **Honest scope**: a structural negative (minimization vacuous; alternator $=$ clamp size bottleneck) that partially deflates L60's "maybe smaller" hope, plus a useful positive (pair-swap involutions exist) and a substrate correction (need $\omega=3$, not triangle-free). No bound moved; the realizable-alternator object is unchanged but its two access routes are now sharp.

---

### L60. First experiments off the L58 phase-gadget route, both decisive. (E1, POSITIVE) The abstract Equality-Alternator EXISTS and is abundant: 693 of 25143 independent edge-pairs of $M^3(C_5)$ (the 47-vtx triangle-free 6-critical host, L51's $H$) are alternators, and one is INDEPENDENTLY verified by all four explicit patterns. This is the $p=2$ phase object the L58 dichotomy needs, the alternator analog of L51's abstract clamp: the route is NOT abstractly vacuous. (E3, NEGATIVE-as-predicted) A naive rational-angle necklace closure of a LINEAGE graph cannot manufacture $\chi\ge6$ for free: the exact $\mathbb{Z}_6$ closure of $P_{510}$ (60-deg rotation about the origin, 868 distinct vertices after 2192 exact orbit coincidences, 5740 edges, $K_4$-free, all cross-bands included) is 5-colorable (instant SAT). Together they relocate the phase route's difficulty precisely: from "does a phase object exist" (YES) to "can an alternator be REALIZED as a UDG with two congruent rotation-related interfaces" (open W3, distinct from the clamp's cocircularity W3).

**Architecture**: 1 (combinatorial). Scripts: [`e9_alternator_abstract.py`](combinatorial/e9_alternator_abstract.py) (sweep) + [`e9_alternator_verify.py`](combinatorial/e9_alternator_verify.py) (independent 4-pattern check, no shared logic); [`e10_necklace_z6.py`](combinatorial/e10_necklace_z6.py) (reuses h7b's exact `cross_unit`). Results under `_cache/e9_alternator/`, `_cache/e10_necklace/`.

**E1 construction (self-certifying).** $K = M^3(C_5)$ is 6-critical, so for any edge $e=(u,v)$, every proper 5-coloring of $K-e$ has $c(u)=c(v)$ (else it would 5-color $K$). For an independent edge pair $(e_1,e_2)$ with 4 distinct endpoints, in $K-e_1-e_2$: (a) both-different is ALWAYS infeasible (such a coloring properly 5-colors $K$, but $\chi(K)=6$); (b) only-$e_1$-same $=K-e_1$ and only-$e_2$-same $=K-e_2$ are ALWAYS feasible (5-colorable by criticality); (c) both-same (contract $e_1$ and $e_2$) is the only question. So both-same 5-UNSAT $\Rightarrow$ every proper 5-coloring has EXACTLY ONE pair same $=$ an equality-alternator, with the two non-adjacent pairs as a width-2 interface carrying a $\mathbb{Z}_2$ phase. No single pair is forced (each is free), so the object is provably INVISIBLE to L45/L56/L57. Verification (e9_verify on $e_1=(0,15),e_2=(9,26)$): both-different False, both-same False, only-$e_1$ True, only-$e_2$ True. PASS.

**E3 why the negative is the right one.** The necklace dichotomy (L58) gives $\chi\ge6$ only if the interface transfer is IMPRIMITIVE ($\mathbb{Z}_p$-phased). A lineage graph has NO phase structure to close on: L57 (all pairs free) and L59 (vertex-critical, no forced pairs) say the lineage carries no nonlocal color correlation, so a rational-angle closure of it stays 5-colorable, exactly as observed (and consistent with h7b: 2- and 3-copy cross-rich unions are 5-SAT). The necklace needs a phase-CARRYING seed (a realized alternator), not a lineage graph. The "free lunch" version of the route (close a known chi-5 UDG) is dead; the live version (realize the alternator first, then close) is unchanged.

**Net effect on the program.** The phase-gadget route is now in the SAME position as the clamp route after L51: abstract object confirmed present, all difficulty in W3 realizability. But it is a DIFFERENT W3 question. The clamp needs $A,B$ unit-cocircular at distinct centers with a degree-5 rainbow split (Theorem R). The alternator needs only two non-adjacent vertex pairs $L$, $R=\tau(L)$ that are CONGRUENT (an isometry relates them) and realized at unit distance with the $\mathbb{Z}_2$ alternation intact, $\tau$ an infinite-order (irrational) or finite-order (rational, for the necklace) rotation. Whether alternator-W3 is easier than clamp-W3 is the next open question; the alternator's interface is width-2 (matching the L55 route-(ii) prediction) and needs no cocircular rainbow, so it plausibly evades Theorem R's degree-5 obstruction.

**Wrong-approach detectors.** E1/E3 are combinatorial decision procedures, no construction, no bound moved. The alternator's abstract existence lifts to $\mathbb{Q}^2$ vacuously (it is pure graph theory); its USE requires the Euclidean realization (congruent unit-distance interfaces $+$ rotation), which is the only detector-relevant content and is exactly what stays open. **Honest scope**: a positive abstract-existence result (E1) plus a predicted negative (E3); no bound moved; the open object is now the realizable equality-alternator.

---

### L59. The Essential-Pair retrodiction CONFIRMED: all nine chi-5 lineage graphs (510, 517, 529, 553, 610, 633, 803, 826, 874) are VERTEX-CRITICAL: ~5145 solves (4-colorability of $G - v$ for every vertex), zero redundant vertices, zero indeterminate. By the Essential-Pair Lemma (L58), a vertex-critical chi-5 graph cannot host a forced non-adjacent pair of ANY kind. So L57's exhaustive freeness (~2.29M pairs, ~4.6M solves) is now a COROLLARY of a one-line lemma plus this ~105-minute scan: the lineage was forcing-sterile BY CONSTRUCTION (SAT minimization drove every graph to criticality, deleting every clamp-capable endpoint before any sweep ever ran).

**Architecture**: 1 (combinatorial). Script [`critscan_lineage.py`](combinatorial/critscan_lineage.py); per-graph checkpoints + `SUMMARY.json` under `_cache/critscan/` (gitignored). P510's criticality was known (L26); the other eight are new, and the L26 result is independently reproduced as a control.

**Significance.** (a) The W3 wall acquires a MECHANISM: the lineage never contained the missing object because its production process provably removes the only vertices that could host it. (b) The search policy of L58 is now load-bearing, not heuristic: chi >= 6 ingredients can only live at chi-redundant vertices, so candidate generation must PRESERVE redundancy (never SAT-minimize), and any future forcing sweep can pre-filter to redundant vertices with an $O(n)$ criticality scan instead of $O(n^2)$ pair tests (a ~450x cheaper gate at lineage scale). (c) Caveat carried from L51: redundancy is necessary, not sufficient (the V=1155 $C_6$ closure is non-minimal yet forcing-free).

**Wrong-approach detectors**: N/A (decision sweep on existing realizable graphs, no construction, no bound). **Honest scope**: confirmatory; no bound moved; the open object is unchanged (a chi-5 UDG with a chi-redundant, forcing-bearing core, or the L58 phase-gadget route that bypasses it).

---

### L58. Creative-attack round 2 (workflow: 10 first-principles lenses, per-proposal adversary, 9/10 lenses delivered, every verdict "wound" = kernel survives): the round breaks the single-funnel picture for the first time. Four products: (a) a W3-FREE certificate family (phase gadgets / closed rotation necklaces whose certificates are BORN-realized, no cocircularity step); (b) a PROVED elementary lemma (Essential-Pair) explaining why the lineage is forcing-sterile: SAT minimization toward vertex-criticality deletes every clamp-capable endpoint, so much of L45/L56/L57 was predetermined by the production process; (c) four lineage sectors no sweep ever searched (non-vertex rich centers, pair-of-pairs 7-bit profiles, symmetry sectors, toroidal quotients); (d) an ERRATUM: S199, L403, T721 have $\chi = 4$ (h6 triage table in this file), so the "12 chi-5 UDGs" phrasing in L45/L56/L57 is wrong for 3 of the 12; the chi-5 members are 510, 517, 529, 553, 610, 633, 803, 826, 874. The L45/L56/L57 headline (every non-adjacent pair FREE in all 12 graphs) is unaffected.

**Architecture**: cross-architecture (1, 2, 4). Workflow `hn-first-principles-attack` (10 builder lenses, adversary per proposal, synthesis done in the main loop to conserve tokens). Raw material: `experiments/orchestrator_sessions/2026-06-09-lens-dossier/` (26 cached structured results + `COMPACT_DOSSIER.txt`). The statmech lens failed twice (agent harness loop), never delivered.

**(a) The Phase-Gadget Dichotomy (Borel + topology lenses, independently convergent; survived hostile re-derivation as a conditional theorem with one fix: EVERY recurrent SCC of the transfer digraph must be imprimitive, not just one).** Let $P$ be a finite UDG with congruent non-adjacent interface tuples $L$ and $R = \tau(L)$, $\tau$ a rotation, and suppose the extendable 5-color transfer digraph of the chain $\ldots, P, \tau(P), \tau^2(P), \ldots$ maps onto a directed $p$-cycle, $p \ge 2$. If $\tau$'s angle is a rational multiple of $\pi$, the closed $N$-necklace is a finite UDG by construction and $p \nmid N$ makes it 5-UNSAT: a finite $\chi \ge 6$ UDG with no realizability step. If irrational, the winding cocycle kills every measurable 5-coloring: $\chi_m \ge 6$ and $\chi_B \ge 6$. Either way a bound moves ("the wall is the weapon"). Minimal target object: the **Equality-Alternator**, a gadget whose every proper 5-coloring satisfies exactly one of $c(u_1) = c(u_2)$ and $c(v_1) = c(v_2)$ on two non-adjacent pairs; width-2 interface (exactly L55 route ii), and provably INVISIBLE to L45/L56/L57 (both marginals free). Decidable per candidate by 2 SAT calls. Cheapest gate: abstract existence on small cells with a $k = 7$ soundness control (any stealth phase at $k = 7$ contradicts the hex coloring and flags a bug).

**(b) Essential-Pair Lemma (proved, adversary re-verified line by line).** If $(s,t)$ is a forced (same or different) non-adjacent pair in a 5-chromatic $G$, then $\chi(G - s) \ge 5$ and $\chi(G - t) \ge 5$. Proof: if $G - s$ is 4-colorable, color it with 4 colors, give $s$ color 5 (now $c(s) \ne c(t)$); since $s, t$ are non-adjacent and all their neighbors use colors 1-4, recolor $t$ to 5 as well (now $c(s) = c(t)$); both patterns realized, so $(s,t)$ is free. Corollary: vertex-critical chi-5 graphs host NO forced pairs of any kind; every clamp endpoint is a chi-redundant vertex. P510 is vertex-critical (L26), and the lineage was produced by minimization toward criticality. Search policy flips: NEVER minimize; generate 5-chromatic UDGs with large chi-redundant cores. First experiment: criticality scan of the other 8 chi-5 lineage graphs (`critscan_lineage.py`, launched with this entry).

**(c) Four unsearched sectors (all cheap, all decisive).** (1) Rich-center rainbow scan: L45/L56/L57 tested only pairs of EXISTING vertices, but Theorem R's clamp endpoints are NEW points at circle centers; scan non-vertex points with $\ge 6$ unit-incident vertices (one SAT call per center by $S_5$ symmetry); any UNSAT is instantly a $\|V\|{+}1$-vertex 6-chromatic UDG. (2) Two-point profile sweep: pairs of disjoint unit edges classified by 7-bit $S_5$-orbital profile; the verified D4 stacking theorem reduces W3 to: find a 5-colorable $K_4$-free plus-framed gadget with dis present, one overlap bit and one transporter bit missing (graded 0-7 SAT fitness; fail-safe: collisions only help; adversary exhibited abstract $n = 11$ gadgets with stacking-sufficient deficiency at $k = 4$). (3) Twist spectroscopy: does any lineage graph have a geometric symmetry $\sigma$ with no $\sigma$-invariant proper 5-coloring (a sector imposing $\sim n$ simultaneous equalities, never tested)? (4) Toroidal wrap sweep: rank-1 quotients $G/\mathbb{Z}v$ with wrap edges; forcing invisible to single contractions.

**(d) New named conjectures.** **Aperiodic Chromatic Rigidity**: no doubly periodic proper 6-coloring of the plane exists (every loop-free flat-torus UDG needs 7); so if $\chi = 6$, every witness coloring is aperiodic, which is WHY nobody can write one down. Refutable by one SAT-found torus 6-coloring. **Constructible Plateau**: $\chi(C^2) = 5$ for $C$ the real constructible (2-power Galois) numbers; the whole lineage lies in $C$ (every spindling step is quadratic), so the 6-jump needs a coordinate with non-2-group Galois closure; adversary confirms the question appears unnamed in the literature. **Conflation** (contrarian anchor, kept as falsification target): $\chi = 5$ in ZFC while $\chi_m \ge 6$.

**Measurable thread: two routes around the sparse-solver wall.** (1) The SE(2) 2-point proposal is DEAD as stated (collapses by hand to the e3c/L9 LP, value 3.72: under $S_k$ symmetry + leg marginals + Graf bridge, apex-summed spin blocks are implied by the abelian constraints); the survivor is PER-APEX-COLOR matrix-Herglotz spin blocks, the minimal genuinely 3-point noncommutative constraint, near-LP cost in cvxpy. (2) Periodic spectral collapse: $L$-periodic measurable colorings have Fourier support on the dual lattice, collapsing the 48342-variable order-2 system to $O(10^3)$ variables in plain cvxpy; rules out only periodic measurable 6-colorings but exercises the blocked order-2 machinery now.

**Other adversary kills worth remembering.** "Rigidity-independent flexible clamp engine" is a provably EMPTY class (Kostochka-Yancey $\|E\| \ge 2.8n - 1.8$ for 6-critical $H$ vs Laman $\le 2n - 5$: clamp engines are always over-braced); FWC-style freezing-threshold asymptotics conflate clustering with global forcing; necklace experiments must seed from the chi-5 nine, not S199.

**Honest scope.** No bound moved; no experiment had run at write time (criticality scan launched alongside this entry). **Wrong-approach detectors**: the phase route is rotation-specific (silent on $\mathbb{R}^1$), its Euclidean content is the exact-angle algebra of two-circle intersections (absent in $L^\infty$), and it claims nothing on $\mathbb{Q}^2$; the Essential-Pair Lemma is pure combinatorics and claims no bound anywhere (legitimately lifts everywhere).

---

### L57. EXHAUSTIVE upgrade of L45 + L56: classifying EVERY non-adjacent pair (not a sample) across the chi-5 UDG lineage confirms there is NO long-range color forcing of ANY kind. NOW COMPLETE at 12/12 graphs: all 2,309,264 non-adjacent pairs classified (S199, L403, 510, 517, 529, 553, 610, 633, T721, 803, 826 = 1,932,224; plus 874 = 377,040), every single one FREE -- 0 forced-different, 0 forced-same, 0 indeterminate, 0 hits. In the realizable lineage, all color forcing is adjacency-local; the missing chi>=6 ingredient is not hiding anywhere in it. *(Erratum, see L58: 3 of the 12 graphs (S199, L403, T721) have $\chi = 4$, not 5; the chi-5 members are the nine numbered graphs. The all-pairs-FREE result stands for all 12.)*
...
**Splice lemma (Part 1, exact).** If $(s,t)$ is a forced-same non-adjacent pair in a UDG $G$, add a new vertex $u$ at unit distance from $t$ but not $s$; then every proper 5-coloring has $c(s)=c(t)\ne c(u)$, so $(s,u)$ is a NON-ADJACENT forced-different pair (a clamp), and contracting $s=u$ forces $t\ne t$ (5-UNSAT, $\chi\ge6$). Verified by brute force on $K_6-e$ (endpoints forced-same; spliced pair non-adjacent, forced-different; contraction 5-UNSAT). So a realizable forced-same gadget yields a realizable clamp and a $\chi\ge6$ UDG: sub-question (i) genuinely suffices.

**The test (Part 2, dual of L45).** L45 tested forced-DIFFERENT by MERGING $(a,b)$ (force same) and checking 5-UNSAT. Forced-SAME is the mirror: ADD an edge $(a,b)$ (force different) and check 5-UNSAT; UNSAT $\Rightarrow$ no proper 5-coloring has $a\ne b \Rightarrow$ forced-same. Swept the same lineage (510, 517, 529, 553, 610, 633, 803, 826, 874, L403, S199, T721): high-degree core exhaustively plus random non-adjacent pairs, ~22.5k pairs total, every solve decisive (0 indeterminate). Result: ZERO forced-same pairs anywhere.

**Consequence.** L55 sub-question (i) (a planar $\omega\le3$ UDG forcing two non-adjacent vertices to the SAME color) is NEGATIVE on the entire realizable lineage, exactly as the W3 wall predicts (a forced-same pair would, by the splice lemma, have been a realizable clamp). The remaining route is L55 sub-question (ii): a genuinely WIDE-interface ($\ge2$-vertex separator) gadget with an imprimitive boundary-coloring transfer matrix, i.e. Theorem R's degree-$\ge5$ split. The open object is unchanged: a NEW chi-5 UDG outside the de Grey/Polymath lineage. **Wrong-approach detectors**: N/A (a search over realizable graphs, no new construction); the negative result is consistent with $\mathbb{Q}^2$ ($\chi=2$, no forcing at all). **Honest scope**: a negative result that completes L45 (it supplies the missing "same" half); no bound moved.

---

### L55. Wildcard B (renormalized clamp) executed: the 2050-brainstorm's "RG leading eigenvalue $> 1$" is the WRONG forcing diagnostic ($\lambda_1$ is the coloring-entropy growth rate, i.e. MANY colorings, the opposite of forcing); the right diagnostic is IMPRIMITIVITY / spectral gap of the color-port transfer matrix. Running it yields a structural result: color-symmetry confines single-vertex-port relations to the primitive monoid $\{0, I, J{-}I, J\}$, so the clamp question FACTORS into (i) a forced-SAME gadget under $\omega\le3$, or (ii) a wide ($\ge2$-vertex separator) interface -- which is exactly Theorem R's degree-$\ge5$ split. No bound moved; a reduction.

**Architecture**: 1 (combinatorial). Script `experiments/combinatorial/wildcard_b_rg_transfer.py` (boolean algebra exact, no floating point; eigenvalues reported only as spectral-gap commentary). Runs clean.

**Result 1 (the monoid obstruction).** A gadget's single-vertex-port relation $R\in\{0,1\}^{k\times k}$ ($R[a,b]=1$ iff some proper $k$-coloring colors the left port $a$, the right port $b$) must be COLOR-SYMMETRIC: relabeling any proper coloring by $\pi\in S_k$ stays proper, so $R[a,b]=R[\pi a,\pi b]$. By 2-transitivity of $S_k$ the only such relations are the four $\{0,\ I,\ J{-}I,\ J\}$ (determined by a diagonal and an off-diagonal value), and these are CLOSED under boolean product (a monoid: $I$ is the unit, $(J{-}I)(J{-}I)=J$ for $k\ge3$, $J$ absorbs). All four are primitive-or-trivial. The permutation / "color-shifter" relation (right $=$ left $+1 \bmod k$) that WOULD give rich $\mathbb{Z}_k$ long-range forcing is NOT color-symmetric, so no gadget realizes it. Hence a CHAIN of single-vertex-port gadgets keeps the $s$-$t$ relation inside the monoid and forces $s\ne t$ only by composing to $J{-}I$. This is the structural ceiling on the "two halves $+$ bridges" / pairwise-port mechanism (L14-L20).

**Result 2 (factorization of the clamp question).** Composing to $J{-}I$ needs either a direct differ-edge (adjacent; trivial) or a forced-SAME ($I$) sub-gadget spliced to a differ-edge. So a clamp (non-adjacent forced-different pair) requires EITHER (i) a planar UDG that forces two NON-adjacent vertices to the SAME color (realizes $I$) -- itself open, and obstructed-looking under $\omega\le3$ -- OR (ii) a genuinely WIDE interface ($\ge2$-vertex separator) whose boundary-coloring transfer matrix is imprimitive without collapsing into the monoid. Case (ii) is EXACTLY Theorem R / L53's degree-$\ge5$ split: the $A/B$ subsets of $N(w)$ are a width-$\ge2$ separator, which is the structural reason the split vertex must have degree $\ge5$.

**RG corollary.** Inflation/substitution CONSERVES the monoid: it conserves but cannot CREATE imprimitivity, so the substitution route is not a free lunch and helps ONLY through case (ii). The Fibonacci-substitution check ($a\to ab,\ b\to a$ on port relations) confirms it: two 'differ' tiles stay primitive at every inflation level (forcing dies at range 1); only a (non-realizable) shifter tile stays imprimitive.

**Calibration.** $C_5$ brute-forced (30 proper 3-colorings): every forced-different pair is ADJACENT, zero non-adjacent forced-different pairs. Reproduces L45 and the R5 refutation independently.

**Honest scope.** No bound moved; this is a structural reduction plus a correction to the L54 wildcard framing. It sharpens the W3 target from "long-range forcing" (vague) to two concrete objects: (i) a planar forced-SAME gadget under $\omega\le3$, (ii) the smallest W3-realizable wide-interface gadget with an imprimitive boundary-coloring transfer matrix. **Wrong-approach detectors**: N/A (no construction). The obstruction is about color-class symmetry, not geometry, so it lifts to $\mathbb{Q}^2$ vacuously; the geometric realizability (W3) it points at is untouched and remains the wall.

---

### L54. Backward-from-2050 retrodiction synthesis: an 8-world + 5-wildcard stress-test (adversary-scored) puts the single most-likely terminal answer at $\chi(\mathbb{R}^2)=6$ via a finite UDG (Architecture 1), and identifies ONE linchpin object shared across the high-credence worlds: a unit-distance-REALIZABLE flexible color-clamp (the W3 object of L51-L53). NOT a new bound; a calibration of where the program's probability mass sits and which object cracks the most worlds.

**Architecture**: cross-architecture (all 4). Meta-synthesis, not an experiment. No code, no certificate, no bound moved.

**The exercise**: 8 retrodicted 2050 "solution worlds" (one per likely terminal state) each carried an adversary verdict with a `plausibility_2050` score and a three-detector ($\mathbb{Q}^2$/$L^\infty$/$\mathbb{R}^1$) pass/fail, plus 5 wildcard ideas with a `kernel_of_truth`. Adversary plausibility scores (out of ~100, calibrated low by the no-buffer prior): `borel` (definability split, $\chi=5<\chi_B=7$) **7**, `meas6` (Fourier $\chi_m=6$ then $=\chi$) **6**, `udg7` ($\chi=7$, Isbell tight) **5**, `col5` ($\chi=5$, 5-coloring exists) **5**, `indep` (ZFC-independence of $\chi$) **5**, `aperiodic` (diffraction $\chi_m=6$) **4**, and the two cleanest-story worlds `udg6` / `ai` (both $\chi=6$ by a realizable clamp) **9** each.

**The ranking, after de-rating the adversary's surface scores for shared-fate and detector quality** (a low absolute score that survives the no-buffer prior beats a higher one that fails a control): MOST LIKELY = **$\chi(\mathbb{R}^2)=6$, lower bound by a finite UDG** (the `udg6`/`ai` cluster: same terminal state, same linchpin, two discovery routes - human-staged vs machine-first). This world (i) passes all three detectors cleanly and FOR THE RIGHT REASON (cocircularity-at-unit-radius is Euclidean-specific, lifts to nothing on $\mathbb{Q}^2$ where $\chi=2$, has no $L^\infty$ analog, is rotation-sensitive so silent on $\mathbb{R}^1$), and (ii) routes through the object this repo has ALREADY isolated as the sole integer gate (L51-L53's W3). Its declared fatal flaw is honest and is exactly our open problem: it ASSUMES the realizable clamp exists. DARK HORSE = `borel` (the definability split), which earns more credence than its 7 because it is the only world that does NOT consume the missing chi-6 UDG as a load-bearing input on its headline invariant; its adversary-flagged fatal flaw (manufacturing $\chi\le5$ from a lower-bound conjecture, plus a norm-blind Borel lower blade) sinks the *stated* form but not the underlying possibility that the right question is $\chi_B$, not $\chi$.

**THE ONE LINCHPIN (cross-world).** A unit-distance-realizable flexible color-clamp: a $K_4$-free 6-critical $H$ with a degree-5 vertex $w$ such that $H-w$ realizes in $\mathbb{R}^2$ with a chosen 3-subset of $N(w)$ at circumradius EXACTLY 1 (equivalently Theorem R / L53: the split pair $(s,t)$ at two DISTINCT unit-circle centers $s^\*\neq t^\*$). It is the explicit chi-6 lower-bound engine of `udg6` and `ai`; the measurable worlds (`meas6`, `aperiodic`) and the axiomatic worlds (`indep`, `borel`) all FALL BACK to it in their adversary steelman fixes (every steelman that survives its detectors re-imports "first build a realizable clamp, then transfer via Falconer / Bernshteyn local-to-Borel"). So one object cracks $\ge5$ of the 8 worlds. This is precisely the repo's named blocker; the retrodiction adds no new physics, it CONFIRMS the blocker is correctly placed and quantifies that ~5/8 of the credible 2050 endpoints run through it.

**Value call (decisive, calibrated).** Probably **6**, weakly. The structural prediction pulling toward 5 (de Grey's 2026 $\omega\le3$ conjecture, if tight) is real but the clamp ESCAPES its premise (forces a non-adjacent PAIR, never a unit-clique; L53), so $\omega\le3$ does NOT cap $\chi$ at 5; that single fact is why the mass sits at 6 not 5. The case for 7 (`udg7`) requires an unbounded cap-free forcing ladder the adversary correctly shreds as an asserted non-sequitur (composition is the EASY part; co-realizability is what stays open and gets HARDER with depth). The case for 5 (`col5`) requires a measurable 5-coloring no one can build and inverts the Croft floor. So: 6 most likely, 5 second, 7 third; and whichever it is, the lower-bound side resolves through W3 realizability.

**The two wildcards worth pursuing** (ranked by kernel-of-truth against the repo's actual walls): (1) **Monotile/substitution forcing** - use the hat/spectre aperiodic matching rules as a SCAFFOLD that manufactures long-range forcing (the thing L45 proves the entire realizable lineage lacks), placing a per-tile gadget so matching-rule propagation = color forcing across distance with no direct edge. Cheap first experiment is runnable NOW with the existing UDG + python-sat stack: a 2-3 tile hat patch in $\mathbb{Q}(\sqrt3)$ with per-tile chi-5 gadgets, SAT-check chi, then test L51's contraction-UNSAT on the forced pair. (2) **Renormalized clamp** - define an RG/transfer-matrix step on a substitution tiling that tracks an effective "forcing coupling" across one inflation; a leading eigenvalue $>1$ would mean forcing is RELEVANT and accumulates across scales even though it is zero at every finite scale checked (L45/L51). The go/no-go is a single spectral-radius computation on a 2-level patch, also runnable now. Both target the EXACT L45/W3 gap and both are intrinsically Euclidean (algebraic inflation ratios, $\sqrt3$ angles) so they dodge the $\mathbb{Q}^2$/$L^\infty$ detectors by construction. NOT pursued in this session (no code written); logged as the highest-value next combinatorial probes.

**Honest scope.** No bound moved. No experiment run. This is a synthesis/calibration entry: it integrates an adversary-stress-tested retrodiction set into the canonical narrative, confirms the W3 realizable clamp as the cross-world linchpin (consistent with L51-L53, not in tension), and surfaces two runnable wildcard probes (monotile-forcing, renormalized-clamp) that attack the L45 long-range-forcing gap from outside the de Grey/Polymath lineage. The de Grey 2026 $\omega\le3$ conjecture is recorded as the live rival hypothesis (predicts 5); the clamp's escape from it (L53) is the reason the synthesis lands on 6.

**Wrong-approach detector status.** N/A (no construction, no bound). The integrated worlds were themselves detector-screened: `udg6`, `udg7`, `meas6`, `aperiodic`, `ai`, `col5`(partial) pass; `indep` and `borel` FAIL the $L^\infty$ control on their headline mechanism (norm-blind Borel/Steinhaus lower bounds that would over-prove $\chi\ge7$ on the $L^\infty$ plane where $\chi=4$), which is exactly why they are dark-horse-at-best, not front-runners.

---

### L53. The structural route to W3 gives an EXACT reduction generalizing L42's single-hub Lemma L (adversary-reviewed, two errors fixed before recording), and it shows the clamp ESCAPES the unit-$K_k$ impossibility that closes the single-hub route. **Theorem (R):** a clamp $G$ (split of a $K_4$-free 6-chromatic $H$ at $w$ into non-adjacent $s\!\sim\!A$, $t\!\sim\!B$) is unit-distance realizable IFF $H-w$ is realizable with $A$ and $B$ each on a radius-1 circle, with **distinct** centers $s^\*\neq t^\*$. The escape: the clamp needs only the PAIR $(s,t)$ forced-different (two colors), never a rainbow-forced 5-set, so it never requires a unit-clique (Lemma L died on rainbow $\Rightarrow$ unit-$K_k$, $\omega=3$); geometrically $s^\*=t^\*$ would collapse $A\cup B$ onto ONE circle with $s=t$ = the single-hub config, forbidden by injectivity, so "two distinct centers" is the clamp's defining feature. Every clamp has $|A|+|B|=\deg_H(w)\ge5$ ($w$ lies in a 6-critical subgraph of $H$, else it sits inside $G$ and $\chi(G)\ge6$).

**Architecture**: 1 (combinatorial / rigidity). Structural follow-up to L52; the route the program chose over installing nauty.

**Experiment / artifacts**: [`W3_CLAMP_REDUCTION.md`](combinatorial/W3_CLAMP_REDUCTION.md) (statement + proof of both directions + the Lemma-L generalization); [`w3_clamp_cocircularity.py`](combinatorial/w3_clamp_cocircularity.py) (verification: edge decomposition $213+8+15=236$, $|A|+|B|=23\ge5$, unit-$K_4$ impossible, two-circle split satisfiable in isolation, codim $1$ for a degree-5 split vs $19$ for the Mycielski apex; result [`_cache/w3_clamp_cocircularity.json`](combinatorial/_cache/w3_clamp_cocircularity.json)). Adversary-reviewed (agent afbae70): caught two ERRORS -- the $s^\*\neq t^\*$ role was inverted (it is load-bearing, $=$ the escape, not a measure-zero nuisance; the "$|A|\le2\Rightarrow$ 1-parameter center" claim was numerically false) and the 6-critical justification was garbled -- plus overclaims; ALL fixed before recording.

**Honest boundary (the load-bearing caveats).** (a) (R) is ELEMENTARY: the defining UDG property ("neighbors lie on the unit circle") specialized to a non-adjacent split pair so the two centers decouple; immediate to a rigidity person. (b) Cocircularity is CLASSICAL (L42: $K_{2,3}$ forbidden, two unit circles meet in $\le2$ points). (c) The residual is the IDENTICAL open object already named in [`rigidity_w3_escape.py`](combinatorial/rigidity_w3_escape.py) (the dossier's A5) and in L51/L52 -- a "flexible color-clamp." (d) codim counts EQUATIONS, not solvability: a generically minimally-rigid $H-w$ has zero internal flex, so even the degree-5 codim-$1$ case is generically UNSATISFIABLE; degree-5 is gated on the SAME unproven flex as the apex's $19$ equations, just a smaller deficit. (e) It REFRAMES W3 into rigidity-theoretic language; no rigidity theorem is applied and NO bound moves. The value is the exact iff + the precise residual ("does $H-w$'s rigidity matroid leave one cocircularity equation satisfiable on a degree-5 split vertex?").

**Wrong-approach detector status.** PASS all three. The abstract reduction lifts to $\mathbb{Q}^2$ ($\chi=2$) but claims no bound there (legitimate, purely combinatorial). The escape uses only abstract normed-plane "unit circle" structure but claims only NON-impossibility, not a construction or bound, so it cannot over-prove on $L^\infty$ ($\chi=4$). The genuinely Euclidean content (is the cocircularity locus MET by a flexing $H-w$?) is exactly what stays unproven -- the correct place for it. No bound produced.

---

### L52. The W3 realizability of the abstract clamp (L51) is NOT decidable by counting, and easy numerics give only evidence: the rigorous route is the L42 cocircularity (Lemma L) algebra. A unit-distance embedder CALIBRATED to recover same-scale realizable graphs from scratch (Kamada-Kawai/spring init + trust-region refine; recovers the Moser spindle and a 61-vertex triangular-lattice patch to $<5\times10^{-11}$) FAILS to realize the 48-vertex Mycielski clamp (best max edge-length error $0.36$ over 60 starts; residuals concentrate on the degree-20/22 apex edges). That is EVIDENCE of non-realizability, with two honest caveats: numerics never PROVE non-realizability, and a lattice is an easier target for graph-layout init than an apex-heavy clamp (so the calibration only partly transfers). **The decisive content is a clean NEGATIVE about method**: both cheap obstruction heuristics are INVALIDATED by the realizable $P_{510}$ -- over-determination $\|E\|-(2\|V\|-3)$ is $+1487$ for $P_{510}$ (realizable) vs $+143$ for the clamp, and max degree is $36$ vs $22$ -- so neither the DOF count (the L42 "$\sim$5000 eqs in 3 unknowns" heuristic) nor vertex degree predicts non-realizability.

**Architecture**: 1 (combinatorial / rigidity). Direct follow-up to L51's clamp.

**Experiment**: [`realizability_w3_clamp.py`](combinatorial/realizability_w3_clamp.py) (result [`_cache/realizability_w3_clamp.json`](combinatorial/_cache/realizability_w3_clamp.json)); calibration PASS (`global_recovery_ok=True`: Moser + lattice recovered from scratch and from perturbed-truth), clamp FAIL (evidence, not proof).

**What it settles, and the next rung.** W3 cannot be decided by counting; it is the ALGEBRAIC SOLVABILITY of the cocircularity equation system (the L42 Lemma-L lineage: a unit-distance vertex's neighbors are cocircular, two unit circles meet in $\le2$ points, etc.). The rigorous tool is exact elimination (Groebner, as in [`h2_groebner_moser14.py`](combinatorial/h2_groebner_moser14.py)), feasible only to $\sim14$ vertices -- but the min clamp order is $\ge9$ (L51 atlas floor) and the explicit Mycielski clamp is 48, with its degree-22 apex a CONSTRUCTION ARTIFACT (not intrinsic; $P_{510}$ realizes at degree 36). So the precise next rung is a SMALL or LOW-DEGREE clamp (split a small K4-free 6-critical graph; candidate: Schrijver $SG(14,5)$, triangle-free $\chi=6$ vertex-critical, bounded degree) on which exact realizability is decidable, OR generalizing L42's single-hub cocircularity obstruction to the clamp's distributed forcing. The open question sharpened: can a sparse, REALIZABLE graph host a clamp, which the realizable lineage (L45) provably never does?

**Wrong-approach detector status.** The realizability step IS the entire Euclidean content: the abstract clamp lifts to $\mathbb{Q}^2$ (where $\chi=2$), so it is meaningless there; everything rides on the $\mathbb{R}^2$ unit-circle geometry the $L^\infty$ detector demands (cocircularity is exactly the Euclidean-specific rigidity, absent in the $L^\infty$ plane where $\chi=4$). No bound moved.

---

### L51. A creative-attack sweep (8 mathematical lenses, externally prior-art-checked and adversarially pruned, 23 agents) re-confirmed from 12 distinct angles that every surviving direction bottoms out at the SAME two walls -- integer: a UDG-realizable non-adjacent forced-different pair (L42/L45); measurable: a custom sparse conic backend for generic $X_{23}$ order-2 (L48/L50) -- and in doing so produced TWO new coordinates. **(1)** The $V=1155$ $C_6$ closure of $P_{510}$ (symmetric, non-minimal, EXACTLY realizable, $\chi=5$ confirmed) has NO long-range forcing either: 1774 structured+random non-adjacent pairs, 0 forced-different, 0 indeterminate. This upgrades L45 beyond the SAT-minimized lineage to a symmetric closure, killing the "symmetry/redundancy will manufacture forcing" hope (the lens-1 idea). **(2)** The ABSTRACT L45 ingredient EXISTS and is a ONE-MOVE construction, which RELOCATES the entire $\chi\ge6$-by-coupling obstruction onto W3 (unit-distance realizability) and corrects the workflow adversary's predicted abstract non-existence.

**Architecture**: 1 (combinatorial), atop a cross-architecture survey (all 4). Workflow `hn-creative-attack` (8 builder lenses $\to$ synthesizer dedup $\to$ surveyor prior-art $\to$ adversary-per-idea $\to$ ranked dossier).

**Experiments**: [`lrf_c6_closure_probe.py`](combinatorial/lrf_c6_closure_probe.py) (result [`_cache/lrf_c6_closure_probe.json`](combinatorial/_cache/lrf_c6_closure_probe.json), 364 s); [`lrf_abstract_clamp.py`](combinatorial/lrf_abstract_clamp.py) (the split construction + SAT gates + controls + atlas floor, result [`_cache/lrf_abstract_clamp.json`](combinatorial/_cache/lrf_abstract_clamp.json)).

**The construction (elementary, but it tidies the open problem).** Let $H$ be any K4-free 6-critical graph (here $H=M^3(C_5)$: Mycielskian tower from $C_5$, triangle-free, $\chi=6$, 47 vtx, SAT-confirmed) and $w$ a vertex. Take a proper 5-coloring $\varphi$ of $H-w$ (exists; criticality forces $\varphi(N(w))=$ all 5 colors). Split $N(w)$ by a 2-partition of those colors into $A,B$ and form $G=(H-w)+\{s,t\}$ with $s\sim A$, $t\sim B$, and $s,t$ NON-adjacent. Then (i) $G$ is K4-free (triangle-free here); (ii) $\chi(G)=5$ ($\varphi$ extends: $s$ sees only colors in $A$'s class, a proper subset of $[5]$, likewise $t$); (iii) $(s,t)$ is forced-different in EVERY proper 5-coloring, because identifying $s,t$ reproduces $H$, which is not 5-colorable. **SAT-verified on an explicit 48-vertex triangle-free witness**: $G$ 5-colorable PASS, $G/\{s,t\}$ 5-UNSAT PASS, $(s,t)$ non-adjacent PASS, K4-free PASS. Wrong-approach gate PASS: the controls $K_{3,3}$, path, $C_6$ ($\chi\le3$) host 0 clamps at $k=5$ (a clamp needs $\chi(\text{contraction})\ge6$, impossible on the bipartite/odd-cycle regime), so the test cannot fabricate a bound on $\mathbb{Q}^2$/$\mathbb{R}^1$-type controls.

**What it settles.** L45's open problem ("find a $\chi=5$ UDG with a non-adjacent forced-different pair") splits cleanly: the ABSTRACT object is a one-liner (so it is NOT "absent, needing a new principle" as previously framed; that framing conflated abstract and realizable), and the ENTIRE difficulty is W3 -- is any such abstract clamp unit-distance realizable in $\mathbb{R}^2$? The h5/h6 abstract $\chi=6$ graphs (L27-L30) do not directly supply this: a $\chi=6$ graph has NO proper 5-coloring, so "forced-different" there is vacuous; the split construction's content is that it yields a genuinely 5-CHROMATIC graph (5-colorings exist) in which the pair is forced. Minimum abstract clamp order $\in[9,48]$: rigorous floor 9 (atlas exhausts all 644 connected K4-free graphs on $\le7$ vtx; all are $\le4$-chromatic, so min K4-free 6-chromatic order $\ge8$), explicit ceiling 48; pinning the exact value needs `nauty`/`geng` (unavailable on this Windows host: no wheel, no C compiler).

**Honest scope.** No bound moved; both 2026-06-01 gates stand unchanged. The value is (a) the symmetric-closure no-forcing datum (extends L45), and (b) the conceptual relocation of the integer obstruction to W3 alone, with an explicit SAT-checked abstract witness. The adversary's "already-run" kills (A4 Groebner infeasibility on Moser$\times$Moser+14B; A8 same-color floor faking $\chi\ge3$ on $\mathbb{Q}^2$/$\mathbb{R}^1$; A10 box-complex coind=2 undershooting $\chi=5$) were spot-checked against the filesystem and are real (files `rigidity_w3_escape.py`, `a8_adversary_probe.py`, `a10_boxcomplex_calibration.py`).

**Wrong-approach detector status.** Intrinsically combinatorial; the construction makes PRECISE that all of the Euclidean / $\mathbb{Q}^2$-distinguishing content of $\chi\ge6$ lives in the unrealized W3 step (the abstract clamp lifts naively to $\mathbb{Q}^2$, where $\chi=2$ -- which is exactly why the abstract object alone proves nothing and realizability is everything). No bound produced.
