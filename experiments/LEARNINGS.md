...
**Honest scope / status.** No bound moved yet. The solver is VALIDATED on small configs; the $X_{23}$ solve is launched (checkpointed). Two honest caveats remain on the $X_{23}$ run: (i) first-order convergence is slow (Moser $k=4$ needed ~4000 iterations), and the per-iteration cvxpy x-update over 48342 variables is the wall-clock bottleneck, so an OSQP-cached x-update (factor once, update the linear term, warm-start) is the optimization needed for a fast $X_{23}$ solve; (ii) order-2's PAYOFF is still speculative (L50: no order-2-vs-order-1 separation below $X_{23}$ scale), so $X_{23}$ $k=4$ may return FEASIBLE (order-2's IEC-size-4 reach insufficient even for $\ge5$, redirecting to order-3) or CERTIFY (the lift works; then $k=5$ is the real test). **Wrong-approach detectors**: N/A (solver on the $O(2)$-averaged Euclidean SDP; the e3i/e3j 1D control already guards the LP lineage). Next: read the $X_{23}$ $k=4$ verdict; if the per-iter cost is prohibitive, swap the x-update to OSQP-cached and rerun.
