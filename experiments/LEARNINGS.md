...
**Honest scope.** No bound moved. HEURISTIC negative (local search, not exhaustive, not a non-existence proof): the object may exist at a structure no local move reaches; in particular a SPARSE 6-critical graph that fits the codegree ceiling is arithmetically allowed at $n=47$ ($m\ge130$ feasible) but is not reachable from the DENSE Mycielski seed by these moves. The floor is a local-search floor and varies (bimodal); 359 is the best of 13 restarts, not a canonical invariant. **Wrong-approach detectors**: N/A (in-class construction search on abstract graphs, no UDG embedding, no bound). Next (E15-v2 follow-ups): seed the repair from a SPARSER $\chi=6$ graph than the Mycielski tower (a near-Kostochka-Yancey-extremal 6-critical graph would start with codegree slack), and/or a proper $\chi=6$-preserving SPARSIFICATION operator (remove a non-violating edge that frees a violating one) to escape the criticality rigidity the current move set respects.
