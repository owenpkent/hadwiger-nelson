...
% TODO: author block. Filled with a placeholder; replace before submission.
\author{Hadwiger-Nelson computational program\\
\small (working note)}
\date{June 13, 2026}
...
realizable lineage contains no such forcing, and provably so. We prove an
elementary \emph{Essential-Pair Lemma} which implies that a vertex-critical
...
bound on $\chrom(\R^2)$. The contribution is threefold: (a) an elementary lemma
explaining \emph{why} the realizable lineage is forcing-sterile; (b) an
exhaustive computational census confirming it across the entire lineage; and
...
The following lemma is elementary. We are not aware of this exact statement in
the literature, though it may well be folklore; the proof uses only the
definition of a proper coloring.
% TODO: literature check. We are not aware of this exact statement in the
% literature but it may be folklore.
...
\begin{enumerate}
  \item \emph{The Essential-Pair Lemma is elementary.} Its proof uses only the
        definition of a proper coloring. We are not aware of the exact statement
        in the literature, but it may be folklore; we make no priority claim.
        % TODO: literature check on Lemma 1.
...
% TODO: acknowledgments. This is a computational program note; the census and
% criticality scans were run with standard SAT solvers on a single workstation.
