...
A preliminary computational probe of this question is suggestive but
inconclusive. A local search (simulated annealing over graphs kept $K_4$-free and
...
$\chrom = 5$ at orders as small as $n = 29$ but never reaches $\chrom = 6$, with
the edge density plateauing near $84\%$ of the codegree
ceiling~\eqref{eq:codeg-ceiling} at every $n$. This is only heuristic evidence,
not a non-existence result: a $\chrom \ge 6$ in-class graph may live at larger $n$
or at the denser configurations the search does not reach. It does indicate that
the target object, if it exists in this range, is not easily constructed.
