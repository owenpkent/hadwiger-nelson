...
- [x] Multi-class (joint k-coloring) measurable moment LP, the one un-capped route to chi_m >= 6:
  ...
  - [ ] **X_23 k=4 solve** (launched, `run_x23`, checkpointed): verdict pending -- FEASIBLE (order-2 insufficient even for >=5, redirect to order-3) or CERTIFY (lift works, then run k=5 for the >=6 frontier at the same cost)
  - [ ] **OSQP-cached x-update**: the per-iteration cvxpy QP over 48342 vars is the wall-clock bottleneck; cache the sparse KKT factorization, update only the linear/prox term, warm-start -> fast X_23 iterations. The optimization needed for a quick solve
