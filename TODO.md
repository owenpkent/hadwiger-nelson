...
- [x] **E16 remaining door (UNBLOCKED, executed as E17)**: nauty 2.8.9 built on the Linux/gcc host (`geng_hn` via `combinatorial/e17_build_geng.sh`); E17 exhaustively enumerated the both-free class per $n$ with a custom-pruned geng (incremental $K_4$/$K_{2,3}$ rejection + cherry budget + proved maxdeg/2-connectivity caps), fully calibrated (independent-filter agreement at $n=7/8/9$, Shrikhande extremal cell, Folkman floor, smoke 9/9, hit path pre-validated on $M^3(C_5)$). **VERDICT: NO $\chi\ge6$ member of the class on $n\le16$, exhaustively** ($n=16$ window 43..48: 11,315 both-free graphs, all 5-colorable, 0 hits, 66.3 cpu-h); the smallest, if any, has $n\ge17$. Upgrades the L65/L67/L69 heuristic negatives; confirms $K_{2,3}$ violations load-bearing (L75, `combinatorial/e17_results.md`)
- [ ] **E17 n=17 door**: measured wall $>80$ cpu-days on this host (no mod=4096 sample part finished in 1,700 cpu-s); needs a qualitatively stronger prune or ~100x compute (~a hundred cores for a week). Do not launch the naive split here. **The Gallai-prune option named here is now CLOSED (E18/L76)**: encoded and calibrated 5/5, it leaves the instance SAT at $n=15..18$ and prunes only 18% of the $n=15$ cell against the ~99% needed, and it cannot go inside the generator at all (degrees only grow during augmentation, so "low vertex" is decidable only at $n=\text{maxn}$, making it a post-filter over the cheap coloring calls rather than over generation). Remaining candidates: clique-cover lower bounds, a canonical-form generator specialized to near-extremal codegree-2 graphs, or SAT modulo symmetries
- [x] **E18: second enumerator + E17 replication on a second host** (L76, `combinatorial/e18_results.md`). Windows/msys host calibrated from scratch (gates 9/9, firewall clean, `geng_hn` built via `e17_build_geng_msys.sh`); all E17 gates reproduce exactly; $n=16$ returns exactly 11,315 with the verdict reproduced to the residue (11,291 DSATUR / 24 SAT / 0 hits) and those 24 isomorphic to the committed set 24/24. SAT-side enumerator (CDCL + blocking clauses) EXHAUSTED the $n=15$ cell and matched geng graph-for-graph; $n=16$ at 11,312/11,315 with only-in-SAT 0. New fact: the both-free class with $\delta\ge5$ is empty through $n=11$ and first nonempty at $n=12$
- [ ] **E18 finish the $n=16$ SAT census** (3 classes short of complete): only the $m=43$ cell is outstanding; all other cells are proved exhausted and the census stands at 11,312/11,315 with zero contradictions. **Do NOT re-run `e18_enumerate` on it** -- that approach was tried at 8-way and 64-way splits and hit the measured blocking-clause wall (next item). Close it with an isomorph-free generator, then `e18_merge.py` + `e18_n16_compare.py`; expect 11,315/11,315 and verdict AGREE
- [ ] **E18 gold runs**: re-decide $n=13,14$ with NO symmetry breaking (`e18_sat_class --n 13 --min-edges 35`, likewise $n=14$/38) on an idle machine with headroom. One was retired mid-run at 2.15 GB when host free memory fell to 1.9 GB. Until they land, those UNSATs rest on the sound-and-gated lex symmetry break rather than a break-free re-derivation
- [ ] **E18 next tool (now measured, not just suspected)**: enumeration-by-blocking dies at ~$10^5$ blocking clauses per solver REGARDLESS of how the cell is split. Two attempts confirm the same knee: 8-way branches closed at 156k/543k models but 5 ran past 540k-760k without closing (68 -> 7-10 models/s); 64-way branches closed at 32-42k but the rest crossed 100k and stalled (50-70 -> 8-23 models/s). Branches under ~50k run at full speed; splitting moves branches within that distribution but does not move the threshold, and the cell has a heavy tail either way. The tail is an artefact of blocking each class's ~250 lex-surviving labellings one at a time, so the fix is an enumerator that never emits duplicates: **SAT modulo symmetries** (Kirchweger-Szeider), canonicity checking inside the solver. Use it before any $n=17$ attempt from the SAT side; do not grind the blocking approach further
- [x] **E17 VERIFIER pass** over `combinatorial/e17_results.md`: VERIFIED (4/5 targets VERIFIED + 1 VERIFIED-WITH-CAVEAT, zero blocking findings); recount at $n=7/8/9$ exact (352/2001/15481), $n=15$ cell + 9/9 sampled $n=16$ parts reproduce exactly, both counting lemmas re-derived with no off-by-one; caveat = 24 SAT residues not persisted (`combinatorial/e17_verification.md`)
- [x] **E17 ADVERSARY pass** over `combinatorial/e17_results.md`: PASS, five attack surfaces SOUND (PRUNE/PREPRUNE fuzzed 272,589 fires, 0 valid completions cut; cherry budget exact at the Shrikhande boundary); closed the verifier's residue-persistence caveat, all 24 SAT residues re-derived and 5-colorable across 4 solvers, 0 disagreements (`combinatorial/e17_adversary.md`)
- [ ] **E17 Lean formalization**: formalize the two E17 counting lemmas (maxdeg cap $\Delta\le\lfloor(n-1)/2\rfloor$; cherry budget $\sum_v\binom{\deg v}{2}\le N(N-1)$) in `lean/HadwigerNelson/`; flagged formalization-ready (elementary Mathlib `SimpleGraph`/`Finset` double counting, no geometry needed) per `combinatorial/e17_verification.md`
...
- [x] **Author name/affiliation**: confirmed "Owen P. Kent / Independent researcher" (2026-06-16); applied to both papers + arXiv metadata
