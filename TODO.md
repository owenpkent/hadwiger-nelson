...
- [x] Multi-class (joint k-coloring) measurable moment LP, the one un-capped route to chi_m >= 6:
  ...
  - [ ] **Wrap e3s in a first-order conic solver** (matrix-free PSD projection per block via `apply_A`/`apply_AT` + linear NORM/MARG/Bochner/IEC constraints + Phase-1 |slack| objective): the remaining increment to actually RUN X_23 order-2, k=4 (retest >= 5) then k=5 (open the >= 6 frontier). Per-iteration cost = the operator application (handful of (mult x D)(D x D) matmuls/block), feasible; many iterations => hours per solve. Payoff still speculative (L50)
