...
- [x] **E18 finish the $n=16$ SAT census**: DONE (L77, 2026-08-09). SAT modulo symmetries (`smsg`, built under msys/ucrt64) closed the $m=43$ cell in 45.4 s via `e18_sms_cell.py` (10,583 classes, one model per class, 0 violations); `e18_merge` reports CENSUS COMPLETE and `e18_n16_compare` returns 11,315 vs 11,315, matched 11,315, **AGREE**. L75 caveat (i) fully closed at $n=16$
...
- [x] **E18 next tool**: DONE (L77). SAT modulo symmetries built and validated on this host (calibration 8/8 exact, models-per-class 1.0); it eliminates the measured ~$10^5$ blocking-clause wall outright (the $m=43$ cell: hours-and-never-closed at every split -> 45.4 s exhausted). `smsg` at `~/.local/src/sat-modulo-symmetries/build/src/smsg.exe` (ucrt64 DLLs on PATH), driver `e18_sms_cell.py`. This is the SAT-side enumerator for any $n=17$ attempt; the leftover blocking branches were retired
