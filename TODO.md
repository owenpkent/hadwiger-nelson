...
- [ ] **E18 next tool**: the blocking-clause tail is an artefact of lex-leader symmetry breaking (models-per-class ~250-370 unsplit, ~14 when split on the HIGHEST-indexed vertex pairs). A genuine isomorph-free SAT generator (SAT modulo symmetries, Kirchweger-Szeider) removes the tail entirely and is the obvious upgrade before attempting $n=17$ from the SAT side
