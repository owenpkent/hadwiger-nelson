...
- [ ] **E16 follow-up**: seed the repair from a SPARSER $\chi=6$ graph than the dense Mycielski tower (a near-Kostochka-Yancey-extremal 6-critical graph starts with codegree slack), and/or add a $\chi$-preserving SPARSIFICATION operator (remove a non-violating edge that frees a violating one) to escape the criticality rigidity the current move set respects
