...
|--------------|----------|-------|
| 1. Combinatorial / UDG | W3: realize the clamp. The host must be K4-free 6-critical, K_{2,3}-free (codegree wall L63), and outside the P510 lineage (forcing-sterility L57). Exhaustive now, not just heuristic: no such host exists at $n \le 16$ at all (E17/L75, nauty built); any host has $n \ge 17$, and enumeration walls at $n = 17$. Independently re-derived on a second host by a SAT-side enumerator, and the $n=17$ wall is now diagnosed rather than merely measured: it is the $\Sigma_2$ colorability half, not missing local structure (E18/L76). | **Yes** - the main bet. Route ii: a wide imprimitive interface. |
