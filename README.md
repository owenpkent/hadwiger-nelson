...
A focused **Geombinatorics-style note**, [`paper/main.tex`](paper/main.tex), now drafts this honestly and is submission-ready (compiles clean, 10 pages; bibliography web-verified): "Forcing-sterility of the realizable unit-distance lineage and a codegree obstruction to $\chi \geq 6$." Its contribution is the exhaustive forcing census (L57) plus the "forcing-sterile by construction" application (L58-L59) plus the codegree synthesis (L63), now strengthened by L69 (the codegree violations are load-bearing for $\chi=6$) and a three-direction outlook; the central lemma it uses is attributed to Martin 2009 (no priority claim), and no bound on $\chi(\mathbb{R}^2)$ is moved. The only remaining pre-submission items are confirming the author block and the decision to submit. Separately, the [`hn_solver` white paper](docs/03_research/hn_solver_whitepaper.md) documents the from-scratch solver, its benchmark, and its honest ceiling (no clause learning).
