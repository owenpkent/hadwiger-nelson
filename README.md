...
│   ├── gradient/                GD attack surface: legal-UDG realizer, adversarial co-opt, gadget kill-test
...
Publication tracking and evaluation live in [`experiments/PUBLICATIONS.md`](experiments/PUBLICATIONS.md): a registry + rubric (verification / novelty / significance axes, SHIP/DEVELOP/FOLD/PARK/ARCHIVE verdicts, priorities) over every candidate discovery, with a SHIP gate (wrong-approach detector + adversary pass + number re-check) and a decision log. Bottom line: **nothing moves the known bounds.** See [`docs/PUBLISHABLE_FINDINGS.md`](docs/PUBLISHABLE_FINDINGS.md) for the earlier tiered assessment.

Two papers are **adversary-cleared** and have validated, ready-to-upload arXiv bundles (each compiles standalone without bibtex, the arXiv AutoTeX behavior). Author: **Owen P. Kent** (Independent researcher).

- **[`paper/main.tex`](paper/main.tex)** (10 pp), "Forcing-sterility of the realizable unit-distance lineage and a codegree obstruction to $\chi \geq 6$." A negative structural note: the realizable $\chi=5$ lineage is forcing-sterile by construction (exhaustive $1{,}955{,}948$-pair census, L57-L59), and the missing $\chi \geq 6$ object is pincered between the Kostochka-Yancey floor and the $K_{2,3}$-free codegree ceiling (L63, L69). Adversary pass: zero blockers, every headline number independently re-derived exact, central lemma attributed to Martin 2009 (no priority claim), no bound moved. Bundle [`paper/arxiv/`](paper/arxiv/); venue arXiv math.CO + Geombinatorics.
- **[`paper_solver/main.tex`](paper_solver/main.tex)** (7 pp), "Symmetry-broken SAT self-certifies the plane's $\chi \geq 5$ lower bound (and where it stops)." A tooling note distilled from the [`hn_solver` white paper](docs/03_research/hn_solver_whitepaper.md): a sound symmetry-broken CNF export makes the entire known $\chi(\mathbb{R}^2)\geq5$ program self-certifiable on one workstation (de Grey 1585 $k=4$ UNSAT in **18.1 min**, backed by [`e1u_degrey_symbreak_k4.py`](experiments/combinatorial/e1u_degrey_symbreak_k4.py); $M^4(C_5)$ $k=6$ in 22 s; each with an optional DRAT proof), plus the portfolio law (L64) and an honest structured-vs-phase-transition boundary. Adversary pass: the symmetry-break soundness is watertight (exhaustive model-count, zero false-UNSAT); all numbers backed. Bundle [`paper_solver/arxiv/`](paper_solver/arxiv/); venue arXiv cs.DM + math.CO.

The strongest underlying result (F1, the structural explanation of the $\chi \geq 6$ wall, L42) is real-but-modest: its geometric core is the classical $K_{2,3}$-freeness of unit-distance graphs, and what survives is one clean open problem -- *find a chi-5 UDG with long-range color forcing (a non-adjacent pair forced-different in every proper 5-coloring)*, exactly what a chi-6 coupling needs and what the de Grey / Polymath lineage lacks.
...
- **L73** — a gradient-descent attack surface, built, calibrated, and firewalled ([`experiments/gradient/`](experiments/gradient/)). The governing constraint stated plainly: GD is local and one-sided, so it builds / finds upper bounds (a coloring or embedding) but NEVER a $\chi \geq 6$ lower bound, which needs a SAT/dual certificate; a pure soft-coloring objective lifts to $\mathbb{Q}^2$ ($\chi=2$) and is excluded by design. Four options: (A) adversarial coordinate$\leftrightarrow$coloring co-optimization (inner colorability oracle validated exactly on the triangle and $K_5$; honest negative: naive GDA from a random seed, and seeded from P510, does not reach $\chi > 5$; a faithfulness limit of the soft-adjacency model on dense real UDGs surfaced), (B) a legal-UDG realizer that patches the legacy residual-only solver with a non-edge margin -- applied to the L63 host candidates, every manufactured $K_4$-free 6-chromatic host FAILS to realize and is rigidity over-determined by $+43$ to $+51$ edges, a continuous SAT-free corroboration of the codegree wall, (C) a first-order spectral push spec'd against [`fractional/`](experiments/fractional/) (Burer-Monteiro Lovász $\vartheta$, the one dual-certifying surface), (D) the flexible-but-color-rigid gadget kill-test from [`rigidity_w3_escape.py`](experiments/combinatorial/rigidity_w3_escape.py) made runnable (flexible clamps exist at $k=2$, vanish below $k=5$). No bound moved; runnable infrastructure with every hard claim routed back to SAT.
- **L72** — the order-2 measurable SDP RAN on $X_{23}$ and returned a decisive NEGATIVE. The matrix-free solver (L70-L71) did what dense assembly never could (the affine map is $\sim$195 GiB) and solved $X_{23}$ order-2 at $k=4$: the Phase-1 margin fell $1.24 \to 0.0021$ (a 600x MONOTONE drop with no plateau, the FEASIBLE signature, versus the certifying plateau the Moser $k=3$ control sticks at, 0.278). So order-2 (PSD coupling + inclusion-exclusion congruence up to subset size 4) does NOT certify even $\chi_m \geq 5$. By color-monotonicity (a $k=4$-feasible moment solution zero-pads to a $k=5$-feasible one; $S_k$-symmetrization is lossless), it cannot certify $\chi_m \geq 6$ at $k=5$ either, so the order-2 measurable SDP route is **closed**. This settles, with a measured answer, a question open since L41/L48/L50 ("does the order-2 lift help at $X_{23}$ scale?"): no -- consistent because order-2's IEC-size-4 reach is below the size-5 the single-class IE-LP needed to cross $1/4$ (L36). Redirect: order-3 (a far larger build) or back to the Architecture-1 missing object. No bound moved (a clean negative). Verdict [`e3u_x23_k4_verdict.json`](experiments/fractional/e3u_x23_k4_verdict.json).
- **L70-L71** — the matrix-free order-2 backend: built, validated, and run. After $S_k$ symmetry reduction the order-2 $X_{23}$ SDP still carries $\sim$195 GiB of DENSE affine-map data, and both escape hatches are dead: $X_{23}$ has TRIVIAL geometric symmetry ($\|\mathrm{Aut}\| = 1$, so no further block reduction) and the reduced map is 100% dense (so no sparse assembly), L70. The fix is a MATRIX-FREE operator ([`e3s_order2_operator.py`](experiments/fractional/e3s_order2_operator.py)) that forms only the $D \times D$ moment matrix ($\sim$125 MB) and projects each block, never the 195 GiB map (validated against the dense e3q reference to $\sim$1e-13). A linearized-ADMM solver wrapping it ([`e3u_order2_solver.py`](experiments/fractional/e3u_order2_solver.py), OSQP-direct x-update since cvxpy OOMs at this scale) is validated 3/3 against e3q's dense feasible/certify margins, L71. Bonus: $k=4$ and $k=5$ reduce to the SAME size (the congruence classes of size-$\leq 4$ subsets saturate at $k=4$), so the $\chi_m \geq 6$ frontier costs exactly the same as the $\chi_m \geq 5$ validation.
- **L69** — the codegree violations are load-bearing for $\chi = 6$ (top-down repair). Starting from $M^3(C_5)$ ($n=47$, $\chi=6$) and repairing its $K_{2,3}$ violations while holding $\chi=6$, then sparsifying all the way to the Kostochka-Yancey 6-critical floor ($m = 131$, essentially $2.8 \cdot 47 - 1.8$), leaves all $166$ violating pairs intact: the chromatic and codegree obstructions are the SAME substructure (you cannot shed the violations without destroying the 6-chromaticity they accompany). This completes a three-direction triangulation of the in-class $\chi \geq 6$ object -- greedy-up $\to \chi 4$ (L65), anneal-up $\to \chi 5$ (L67), repair-down $\to$ stall (L69) -- all heuristic negatives. Folded into the paper's codegree section. Artifact [`e16_repair_floor.json`](experiments/combinatorial/e16_repair_floor.json).
- **L68** — the solver ceiling broken, by a different lever than L66 predicted. The named lever (watched-literal nogood propagation, [`hn_solver_wl.py`](experiments/_shared/hn_solver_wl.py), validated correct) was implemented and STILL loses to pure CBJ: a Prosser conflict set is a weak, non-asserting clause, so even propagated in O(1) it cannot repay its per-node cost (the bottleneck was never the scan speed). The actual world-class win is to EXPORT `hn_solver`'s native color-permutation symmetry break as a CNF preamble ([`build_color_cnf_symbreak`](experiments/_shared/portfolio_sat.py), `colorable_portfolio(symbreak=True)`) so a production CDCL engine (Cadical, with real 1UIP learning) inherits it. Validated equisatisfiable with the naive encoding (0 disagreements / 0 bad witnesses, 1000+ instances). It crosses two standing walls: $M^3(C_5)$ $k=5$ UNSAT 7.0 s $\to$ **0.05 s (~146x)**; $M^4(C_5)$ $k=6$ UNSAT (intractable for pure `hn_solver` and the naive portfolio) $\to$ **21.9 s**; $P_{510}$ $k=4$ UNSAT (naive $\sim$110 s) $\to$ **1.66 s**. No bound on $\chi(\mathbb{R}^2)$ moved (solver tooling), but the L64 in-class E14 decisions now have a default that gives every portfolio member the symmetry break. A capability battery makes the **entire known $\chi(\mathbb{R}^2)\ge5$ program self-certifiable**: de Grey's 1585-vertex graph k=4 UNSAT in **18.1 min** (1088 s, symbreak vs 92.2 min naive; an independent confirmation that $\chi(\mathbb{R}^2)\ge5$; backed by [`e1u_degrey_symbreak_k4.py`](experiments/combinatorial/e1u_degrey_symbreak_k4.py)), all nine $\chi$-5 lineage graphs in 1.5-9 s each, each with an optional DRAT certificate. Honest boundary: symbreak crushes structured UNSAT but does NOT tame the E14 overshoot's PHASE-BOUNDARY instances (P510 + $\sim$1000 edges at k=5: undecided even at 10M conflicts for both encodings), so the search for a NEW in-class $\chi\ge6$ object stays hard (intrinsic, not a solver artifact).
